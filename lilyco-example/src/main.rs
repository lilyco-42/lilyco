...
//! imgpress --gui     # Web GUI（SSE 进度）
//! imgpress --mcp     # MCP stdio 服务器（AI Agent 直接调用）
//! ```
...
fn resize(img: DynamicImage, max_w: u32, max_h: u32) -> DynamicImage {
    ...
    img.resize_exact(
        (w as f64 * ratio).max(1.0) as u32,
        (h as f64 * ratio).max(1.0) as u32,
...
// ── main: one line ────────────────────────────────────────
//
// 一个入口，四端自动分发（lilyco facade）：
// - 交互终端 → TUI 表单（起不来自动回退 CLI）
// - 管道/脚本 → CLI（--json / --json-stream 供 AI 消费）
// - --gui     → Web GUI
// - --mcp     → MCP stdio 服务器

...
| `list` | `.list-rows` + N 行（**行的控件按 item 类型**：Text→文本、Number→带区间的数字框、Enum→下拉、Path→等宽文本、Flag→复选框）+ `.list-add` | hover / focus（`.btn-icon` 环）/ disabled\* / loading：n/a（行是本地增删，没有任何等待）/ 空 = 删到最后一行时按模板补一行（实测连删 5 次仍留 1 行，且补出来的仍是**数字框**）/ error·success：n/a（逐行无独立反馈，对错由服务端校验在运行时说） | `--s-2` 节奏 / `--danger`（删除 hover） | 行控件 `aria-label` 取参数说明；新增行 `focus()` 跟上；新行是**克隆 `initList` 存下的服务端模板行**（`ROW_TMPL`），清行按 `data-item-kind` 分支。以前删空后走的是一段手搓的兜底建行代码 —— 于是 `Number` / `Enum` 的行删完变成文本框（JS 里不许有第二份「item→控件」映射，同 §8.1） |

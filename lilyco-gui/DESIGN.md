...
是唯一的闸门（HTML ≤ 21,000 B、CSS ≤ 22,500 B），量的是**提交进仓库的字节**
（先把 `\r\n` 归一回 `\n`）—— Windows 上按 `core.autocrlf` 检出会让磁盘上的同一份文件胖 441 B，
2026-09-23 闸门就因此在 CI 上红过一次（本地 20,751 B / CI 21,192 B）。预算管的是资源本身，
不是某台机器的检出方式；顺带记着：Windows 构建出来的 `.exe` 里嵌的就是那份带 `\r` 的字节。
涨过线编译期就红 —— HTML 那一档只剩 249 B 余量，
...
| `number` | `<input type=number>` + `.field-hint` | 同 `text`，另有 `:out-of-range`（实测填 999 进 `1–100` 的框，`matches(":out-of-range")` 为真，描边由这条规则转红） | 同上 + `--ink-muted` 5.43·7.07 | `min`/`max` 进属性 **且** 写成可见提示；`aria-describedby="hint-<arg>"` 让读屏念得出区间。**单边也要写**（只有 `min=0` 时以前一个字都不报，`number_bounds_are_spoken_whichever_side_they_come_from` 盯着；无区间就不画提示行） |
...
| `list` | `.list-rows` + N 行（**行的控件按 item 类型**：Text→文本、Number→带区间的数字框、Enum→下拉、Path→等宽文本、Flag→复选框）+ `.list-add` | hover / focus（`.btn-icon` 环）/ disabled\* / 空 = 删到最后一行时按模板补一行（实测连删 5 次仍留 1 行，且补出来的仍是**数字框**）/ error·success：n/a（逐行无独立反馈） | `--s-2` 节奏 / `--danger`（删除 hover） | 行控件 `aria-label` 取参数说明；新增行 `focus()` 跟上；新行是**克隆 `initList` 存下的服务端模板行**（`ROW_TMPL`），清行按 `data-item-kind` 分支。以前删空后走的是一段手搓的兜底建行代码 —— 于是 `Number` / `Enum` 的行删完变成文本框（JS 里不许有第二份「item→控件」映射，同 §8.1） |
...
| `result` | `结果` 标题 + `复制` + `<pre>` | empty = 整块 `hidden`（**每次运行开头就清**：`run()` 调 `clearResult()`，不然日志在滚、结果还写着上一次的 JSON）/ success（跑完露出 JSON）/ focus（`<pre>` 自己 `:focus-visible` 内描边）/ hover·disabled·loading：n/a / error：n/a（失败没有结果，也不在跑） | `--surface-2` `--hairline` `--ink`（17.63·14.50）`--mono` `--r-panel` `--s-3` `--s-4` | `JSON.stringify(…,2)` 纯文本、不走 innerHTML；≥1200 限高内滚；横向也溢出，所以和 `#log` 同一条规矩：`role=region` + `aria-label` + `tabindex=0`，键盘能滚（WCAG 2.1.1） |

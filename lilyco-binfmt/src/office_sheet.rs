...
pub struct OfficeSheet {
    ...
    /// 顺带交一份 markdown 表格（第一行当表头，竖线写 `\|`，格内换行写 `<br>`）
    #[arg(about = "Also render one sheet as a markdown table")]
    markdown: bool,

    /// `--csv` / `--markdown` 要哪张表：表名，或 `index` 那个从 0 起的序号；不给就是第一张
    #[arg(
        about = "Sheet for --csv / --markdown: name or 0-based index",
        default = ""
    )]
    sheet: String,
...
fn run_office_sheet(app: &OfficeSheet, ctx: &Context) -> Result<Value, AppError> {
    ...
    if doc.family == Family::Ooxml && doc.app == "excel" {
        ...
        let mut totals = json!({
            ...
            "links": 0, "hyperlink_formulas": 0, "pictures": 0,
        ...
        for (index, one) in root.descendants("sheet").iter().enumerate() {
            ...
            match xml(bytes, &part) {
                Some(member) => {
                    ...
                    let links = xlsx_links(bytes, &part, &sheet_root, limit);
                    // 位图另开一本账：这一族的「图」有两种（chart 与 drawing），各走各的
                    let pictures = xlsx_pictures(bytes, &part, limit);
                    let pictured = pictures["total"].as_u64().unwrap_or(0) as usize;
                    let broken = pictures["unresolved"].as_u64().unwrap_or(0)
                        + pictures["missing_media"].as_u64().unwrap_or(0);
                    entry["pictures"] = pictures;
                    bump(&mut totals, "pictures", pictured);
                    if broken > 0 {
                        notes.push(format!(
                            "这张表上 {} 张图配不上号或跳不到图部件（图还在锚块里，字节不在）",
                            broken
                        ));
                    }
                    let linked = links["total"].as_u64().unwrap_or(0) as usize;
                    entry["links"] = links;
                    bump(&mut totals, "links", linked);
                    let mut cells: Vec<Value> = Vec::new();
                    let mut count = 0usize;
                    let mut formulas = 0usize;
                    let mut hyperlink_formula = 0usize;
                    ...
                    for cell in sheet_root.descendants("c") {
                        ...
                        if let Some(raw) = &formula {
                            formulas += 1;
                            // `HYPERLINK(...)` 是第四种链接存法：它不写 `<hyperlink>` 元素，
                            // 地址写在公式正文里当字符串参数 —— 所以这一本单独数
                            if raw.contains("HYPERLINK(") {
                                hyperlink_formula += 1;
                            }
                        }
                    ...
                    // HYPERLINK 公式那一本要等格子那一轮走完才有数，回填进同一本账
                    entry["links"]["formula_cells"] = json!(hyperlink_formula);
                    bump(&mut totals, "hyperlink_formulas", hyperlink_formula);
    ...
    if doc.family == Family::Odf && doc.app == "excel" {
        ...
        for (index, one) in book.sheets.iter().enumerate() {
            ...
            let charts = hosts
                ...
                .unwrap_or_default();
            let charted = charts.len();
            // 表上那张位图：frame 坐在格子里还是表上按结构判，地址只有一跳
            let frames = hosts
                .iter()
                .find(|(name, _)| *name == one.name.as_str())
                .map(|(_, node)| ods_pictures(node, bytes, limit))
                .unwrap_or_else(|| picture_ledger(None, 0, Vec::new(), 0, 0, 0));
            let framed = frames["total"].as_u64().unwrap_or(0) as usize;
            ...
            let links = ods_links(one, limit);
            let linked = links["total"].as_u64().unwrap_or(0) as usize;
            let by_formula = links["formula_cells"].as_u64().unwrap_or(0) as usize;
            sheets.push(json!({
                "index": index,
                "name": one.name,
            ...
            bump(&mut totals, "links", linked);
            bump(&mut totals, "hyperlink_formulas", by_formula);
            bump(&mut totals, "charts", charted);
            bump(&mut totals, "pictures", framed);
        ...
        let result = json!({
            ...
            "markdown": markdown_report(app.markdown, &grid_names, &grids, &app.sheet, 0),
            "notes": notes,
        });
        ctx.done(result.clone(), start.elapsed().as_millis() as u64);
        return Ok(result);
    ...
    if doc.family == Family::Compound && doc.app == "excel" {
        ...
        if !book.link_counts.is_empty() {
            notes.push(
                "链接这一族另有 0x01B7 一条，它自报一个 16 位数（按写的交，在 \
                 `workbook.links_written` 里）。实测 LibreOffice 那份写 0，\
                 而同一条流里有六条 0x01B8 —— 它显然不是「链接条数」，但那句话是文件\
                 自己写的，这里照它交回来，不替它改成 6。另：这一族的公式是二进制的 ptg\
                 串，`HYPERLINK(...)` 公式那一本判不住，交 null 而不是 0"
                    .to_string(),
            );
        }
        let result = json!({
            ...
            "workbook": {
                ...
                // 0x01B7 那一条自报的数，按写的交（实测这份件写 0，而流里有六条 0x01B8）
                "links_written": book.link_counts,
                // 图的字节待在哪：0x00EB 的条数与正文长度，和从它的嵌套层里走出来的
                // 内嵌图（OfficeArt 的 0xF007）。instance 与字签位置都按文件写的原值交：
                // 实测这个生产者给 PNG 写 6、给 JPEG 写 5，而「签名到正文末尾」的字节数
                // 三条都正好等于当初那张图片 —— 但这个对应只在「这一份件」的意义上成立
                "pictures": {
                    "drawing_groups": book.drawing_groups.iter().map(|(at, cb)| json!({"offset": at, "cb": cb})).collect::<Vec<Value>>(),
                    "blips": book.blips.iter().map(|one| json!({
                        "offset": one.offset,
                        "instance": one.instance,
                        "cb": one.cb,
                        "magic_at": one.magic_at,
                        "kind": one.kind,
                        "inline_bytes": one.inline_bytes,
                    })).collect::<Vec<Value>>(),
                    "total": book.blips.len(),
                },
                "totals": {
                    ...
                    "links": book.sheets.iter().map(|one| one.links.len()).sum::<usize>(),
                    // 记录条数与解出来的条数是两本账，两个都交
                    "link_records": book.sheets.iter().map(|one| one.link_records).sum::<usize>(),
                    "picture_shapes": book.sheets.iter().map(|one| one.picture_shapes).sum::<usize>(),
                    "drawing_records": book.sheets.iter().map(|one| one.drawing_records).sum::<usize>(),
                },
            },
            ...
            "sheets": book.sheets.iter().map(|one| json!({
                ...
                "links": xls_links(one, limit),
                // 这一族每张表只有这两个数：几条「形状 = 图片」的记录、几条画法记录。
                // 锚块与图的字节都不在表子流里（见 workbook.pictures），所以不编一份列表
                "pictures": {
                    "shapes": one.picture_shapes,
                    "drawing_records": one.drawing_records,
                },
...
/// 这张表上的链接。地址有三条来路，而 `<hyperlink>` 元素自己不说它走哪一条：
/// * `r:id` → 这一张表自己的关系表（`TargetMode="External"` 按写的交，没写是 null）
/// * `location` → 站内跳法（`'数据'!A1`），地址就写在元素上，没有第二跳
/// * 两个都写 → 各说各的，两条都交回去
///
/// 所以逐条说清「写了哪个号」「那个号指不指得到」。两个生产者写的属性面不一样：
/// openpyxl 只写 `ref` 与来路，LibreOffice 每条都补一个 `display`，于是
/// `with_display` 在两份件上是 0 对 5 —— 这不是谁读丢了，是文件自己写的差别。
/// 还有第四种来路：`HYPERLINK(...)` 公式，它压根不写 `<hyperlink>` 元素，
/// 所以那份账由格子那一轮数出来再回填进 `formula_cells`（这里先交 null，不猜）
fn xlsx_links(bytes: &[u8], part: &str, root: &xmlscan::Node, limit: usize) -> Value {
    ...
    if let Some(member) = xml(bytes, &rels_name) {
        ...
        for one in rel_root.descendants("Relationship") {
            ...
            pool.push((
                ...
                one.attr("TargetMode").map(|had| had == "External"),
            ));
        }
    }
    ...
    let mut with_display = 0usize;
    let mut with_tooltip = 0usize;
    let mut external = 0usize;
    let mut unresolved = 0usize;
    for one in root.descendants("hyperlink") {
        total += 1;
        let id = one.attr_local("id").map(String::from);
        let location = one.attr("location").map(String::from);
        let display = one.attr("display").map(String::from);
        let tooltip = one.attr("tooltip").map(String::from);
        if id.is_some() {
            with_id += 1;
        }
        if location.is_some() {
        ...
        let hit = id
            .as_ref()
            .and_then(|want| pool.iter().find(|one| &one.0 == want));
        let target = hit.map(|one| one.1.clone());
        let mode = hit.and_then(|one| one.2);
        if mode == Some(true) {
            external += 1;
        }
        // 「指不到」只说这条链接一个字也没落地：有 location 就已经知道跳哪儿了
        if target.is_none() && location.is_none() {
            unresolved += 1;
        }
        if list.len() < limit {
            let hop = if id.is_some() { "rels" } else { "inline" };
...
/// .ods 那一份链接账。这一族没有「站内 / 站外」那个开关（OOXML 的关系表上才有
/// `TargetMode`），所以只能按地址的长相分：`#` 开头的是站内跳法（实测
/// `#'数据'.A1`，注意 ODF 用点、OOXML 与 .xls 用叹号），认得出 scheme 的算站外。
/// `via_*` 两本是回答「这一族到底把链接写在哪」：实测 LibreOffice 两份件
/// 一条 `table:hyperlink` 都不写，六条全在段里的 `text:a` 上
fn ods_links(sheet: &crate::odsheet::Sheet, limit: usize) -> Value {
    let list: Vec<Value> = sheet.links.iter().cloned().take(limit).collect();
    let internal = sheet
        .links
        .iter()
        .filter(|one| matches!(one["target"].as_str(), Some(raw) if raw.starts_with('#')))
        .count();
    let via_text = sheet
        .links
        .iter()
        .filter(|one| one["via"].as_str() == Some("a"))
        .count();
    json!({
        "family": "inline",
        "total": sheet.links.len(),
        "external": sheet.links.iter().filter(|one| !one["scheme"].is_null()).count(),
        "internal": internal,
        "unresolved": sheet.links.iter().filter(|one| one["target"].is_null()).count(),
        "formula_cells": sheet.hyperlink_formulas,
        "via_text_a": via_text,
        "via_table_hyperlink": sheet.links.len() - via_text,
        "listed": list.len(),
        "cut": list.len() < sheet.links.len(),
        "list": list,
    })
}

/// .xls 那一份链接账（0x01B8 记录）。站外 / 站内这一刀不是猜的，是记录自己写的：
/// 两条分支的**长度字段数的东西不一样**（一支数字节、一支数码元），所以判据只能是
/// 「名字串后面那 16 字节是不是第二个 GUID」—— `guid_second` 就是这一刀本身。
/// `id` / `tooltip` 这一族压根没有，交 null；`display` 用记录里那句 friendly name，
/// 它与 OOXML 那个 `display` 是同一类东西（元素上另写的一句名字）
fn xls_links(sheet: &crate::biff::Sheet, limit: usize) -> Value {
    let list: Vec<Value> = sheet
        ...
        .map(|one| {
            ...
            let location = one.location.clone();
            let scheme = target
                .as_deref()
                .or(location.as_deref())
                .and_then(crate::office_slide::link_scheme);
            json!({
                "ref": one.reference(),
                "range": [one.first_row, one.last_row, one.first_col, one.last_col],
                "target": target,
                "scheme": scheme,
                "external": one.guid_second,
                "location": location,
                "display": one.friendly,
                "tooltip": Value::Null,
                "hop": "record",
                "id": Value::Null,
                // 这两个数按写的交：实测外部支是 2 配 23、站内支是 2 配 28，
                // 手上没有第二个读者判得住它们是什么，所以不替它们编规范名
                "at24": one.at24,
                "at28": one.at28,
                "guid_first": one.guid_first,
                "guid_second": one.guid_second,
                "whole": one.whole,
            })
        })
        .collect();
    let external = sheet.links.iter().filter(|one| one.guid_second).count();
    json!({
        "family": "record",
        "total": sheet.links.len(),
        "external": external,
        "internal": sheet.links.len() - external,
        "unresolved": sheet.links.iter().filter(|one| one.target.is_none() && one.location.is_none()).count(),
        // 这一族的公式是二进制的 ptg 串，这一版不反汇编，所以《HYPERLINK 公式》
        // 这一本在这里是「看过而判不住」，交 null 而不是 0
        "formula_cells": Value::Null,
        // 记录条数与解出来的条数是两本账：解不动的那条不硬凑，但「有几条」要说真话
        "records": sheet.link_records,
        "whole": sheet.links.iter().filter(|one| one.whole).count(),
        "listed": list.len(),
        "cut": list.len() < sheet.links.len(),
        "list": list,
    })
...
/// 与相对的 `../...`）由 `resolve_target` 吃掉。交回的四样是「类型结尾的名字、`Id`、解析后的 `Target`、`TargetMode` 是否 External`：画法部件里的图是按 `r:embed="rIdN"` 指过去的，没有号就配不上 —— 只交名字与目标那一版不够它用
pub(crate) fn rels_with_ids(bytes: &[u8], part: &str) -> Vec<(String, String, String, bool)> {
    let dir = match part.rsplit_once('/') {
        Some((head, _)) => head.to_string(),
        None => String::new(),
    };
    ...
    root.descendants("Relationship")
        ...
        .filter_map(|one| {
            ...
            let external = one.attr("TargetMode") == Some("External");
            Some((
                tail,
                one.attr("Id").unwrap_or_default().to_string(),
                // 外部的 Target 是一个 URL，按包内路径解析没有意义，原样交
                if external {
                    target.to_string()
                } else {
...
/// 旧口径：丢掉 `TargetMode="External"` 那几条，只交「类型名 + 解析后的目标」。
/// 批注、图（chart）、表对象那三本用的就是这个
pub(crate) fn rels_of(bytes: &[u8], part: &str) -> Vec<(String, String)> {
    rels_with_ids(bytes, part)
        .into_iter()
        .filter(|one| !one.3)
        .map(|(kind, _, target, _)| (kind, target))
        .collect()
}
...
/// 一份「图」账的公共外壳。`total` 是**看到**的条数（截断之前），`list` 是列出的那些，
/// 所以 `cut` 一摆开就不会把「只列了前 N 个」读成「只有 N 张」。`unresolved` 与
/// `missing_media` 只数列出的那些 —— 截断之后的没看过，不能替它们说没坏。
/// `drawings` 只有 OOXML 那一族有（画法部件的个数），ODF 没有这一层，交 null 不交 0。
/// `other_anchors` 是那一份画法里**根本不是位图**的摆位（图 chart 与嵌入对象占的那些），
/// 单独记一本：把它们当「坏掉的图」数，chart.xlsx 那份就会被报成两张坏图 —— 它一张图也没有
fn picture_ledger(
    drawings: Option<usize>,
    total: usize,
    list: Vec<Value>,
    unresolved: usize,
    missing_media: usize,
    ...
    // 去重按「这一条自己写的地址」：OOXML 那条叫 `target`（关系跳完得到的包内名），
    // ODF 只有一跳、那个键叫 `href`。两族共用这一层，所以两个名字都认，缺哪个都不编
    let mut distinct: Vec<String> = Vec::new();
    for one in &list {
        let raw = one
            .get("target")
            .or_else(|| one.get("href"))
            .and_then(|had| had.as_str());
        if let Some(raw) = raw {
            if !distinct.iter().any(|had| had == raw) {
...
/// 锚块里那一个角的四个数（`from` / `to`）。`absoluteAnchor` 一个角都不写，那一支交 null；
/// 文字能当数就交数（文件写的是 EMU 与列行号，这里不做任何换算）
fn xdr_corner(anchor: &xmlscan::Node, want: &str) -> Value {
    let Some(one) = anchor.child(want) else {
        return Value::Null;
    };
    json!({
        "col": numeric_or_text(one.child("col").map(|had| had.text())),
        "col_off": numeric_or_text(one.child("colOff").map(|had| had.text())),
        "row": numeric_or_text(one.child("row").map(|had| had.text())),
        "row_off": numeric_or_text(one.child("rowOff").map(|had| had.text())),
    })
}

/// 某个**直接孩子**的属性表。没有那个孩子交 null，而不是空表 —— 空表是「有这个元素
/// 但它一个属性都没写」，两件事得分开
fn xdr_attrs_of(node: &xmlscan::Node, want: &str) -> Value {
    node.child(want).map(written_attrs).unwrap_or(Value::Null)
}

/// `spPr/xfrm`：LibreOffice 写（`off` 与 `ext` 各一对 EMU 数），openpyxl 整个不写，
/// 所以它交 null 是这一族的实话
fn xdr_xfrm(sp_pr: &xmlscan::Node) -> Value {
    let Some(one) = sp_pr.child("xfrm") else {
        return Value::Null;
    };
    json!({
...
/// 一个锚块里那张图的全部：摆法、两个角或绝对坐标、形状名与替代文字、`r:embed` 那个号、
/// 顺着号跳到的图部件与它在包里的字节数（按中央目录自报的解压后长度，与 `office-package`
/// 同一口径，不解压）。`external` 是那一条关系自己写的 `TargetMode`，没写就是 false
fn xdr_picture(
    drawing: &str,
    anchor: &xmlscan::Node,
    rels: &[(String, String, String, bool)],
    bytes: &[u8],
) -> Value {
    let pic = anchor.child("pic");
    let shape = pic
        .and_then(|one| one.child("nvPicPr"))
        .and_then(|had| had.child("cNvPr"));
    let blip = pic
        .and_then(|one| one.child("blipFill"))
        .and_then(|had| had.child("blip"));
    let embed = blip.and_then(|one| one.attr_local("embed"));
    let found = embed.and_then(|want| rels.iter().find(|one| one.1 == want));
    let target = found.map(|one| one.2.clone());
    let media = target
        .as_deref()
        .and_then(|want| zipread::find_entry(bytes, want))
    ...
    json!({
        ...
        "shape_id": shape.and_then(|one| one.attr("id")).map(|raw| raw.to_string()),
        "shape_name": shape.and_then(|one| one.attr("name")).map(|raw| raw.to_string()),
        "descr": shape.and_then(|one| one.attr("descr")).map(|raw| raw.to_string()),
        "shape_attrs": shape.map(written_attrs).unwrap_or(Value::Null),
        "blip_attrs": blip.map(written_attrs).unwrap_or(Value::Null),
        "blip_id": embed.map(|raw| raw.to_string()),
        "external": found.map(|one| one.3).unwrap_or(false),
        "target": target,
        "media_bytes": media,
        "xfrm": pic
            .and_then(|one| one.child("spPr"))
            .map(xdr_xfrm)
            .unwrap_or(Value::Null),
    })
}
...
/// 比图（chart）那一本多出来的是**每个锚块自己写的位置**。两个生产者把同一件事说成两种样子：
/// openpyxl 用三种锚块名分别表示跨格 / 单格 / 绝对，LibreOffice 一律写 `twoCellAnchor`
/// 而把区别放进 `editAs` 属性 —— 所以 `placed` 与 `anchor_attrs` 两样都交，只看元素名
/// 就会把同一份件读成不同的摆法。`unresolved` 数的是「号配不上关系」或「压根没写号」，
/// 手上这份件正好有一条关系被删掉而锚块还留着，所以这一本不是空的装饰
fn xlsx_pictures(bytes: &[u8], part: &str, limit: usize) -> Value {
    let mut list: Vec<Value> = Vec::new();
    ...
    for (_, drawing) in rels_of(bytes, part) {
        ...
        drawings += 1;
        let Some(member) = xml(bytes, &drawing) else {
            continue;
        };
        let root = xmlscan::parse_str(&member.as_text());
        let rels = rels_with_ids(bytes, &drawing);
        for anchor in drawing_anchors(&root) {
            // 没有 `pic` 的那个锚块装的是别的东西（图 chart 走 `graphicFrame`）：
            // 它不是坏掉的位图，另记一本，不进 total 也不进 unresolved
            if anchor.child("pic").is_none() {
                others += 1;
                continue;
            }
            total += 1;
            ...
            let one = xdr_picture(&drawing, anchor, &rels, bytes);
            if one["target"].is_null() {
                unresolved += 1;
            } else if one["media_bytes"].is_null() {
                missing += 1;
            }
            list.push(one);
        }
    }
    picture_ledger(Some(drawings), total, list, unresolved, missing, others)
}

/// 一个 `draw:frame` 里那张图的全部。尺寸是**自带单位的串**（`svg:width="1.057cm"`），
/// 摆法没有 OOXML 那种元素名 —— 位置在 `svg:x` / `svg:y`，而「坐在格子里还是坐在表上」
/// 是结构事实（`in_cell`）。`table:end-cell-address` 只有那一条跨格的 frame 写了，
/// 所以它是照交的一个属性，不能当「挂在哪儿」的通用判据
fn ods_frame_picture(frame: &xmlscan::Node, in_cell: bool, bytes: &[u8]) -> Value {
    ...
    json!({
        ...
        "name": frame.attr_local("name").map(|raw| raw.to_string()),
        "x": frame.attr_local("x").map(|raw| raw.to_string()),
        "y": frame.attr_local("y").map(|raw| raw.to_string()),
        "width": frame.attr_local("width").map(|raw| raw.to_string()),
        "height": frame.attr_local("height").map(|raw| raw.to_string()),
        "end_cell_address": frame
            .attr_local("end-cell-address")
            .map(|raw| raw.to_string()),
        "href": href,
        // 同一个 frame 里还有 `draw:object` 的，这张位图是那个图表/嵌入对象的**预览**
        // （LibreOffice 把缓存图写在 `ObjectReplacements/` 那条地址上）：它确实是包里的一张
        // 位图，但它不是「用户放在表上的那张图」，所以这个开关跟着交，不替两边挑
        "also_object": frame.child("object").is_some(),
        "mime": image
            .and_then(|one| crate::odsheet::attr_of(one, "mime-type"))
            .map(|raw| raw.to_string()),
        "image_written": image.map(written_attrs),
        "alt": frame.child("desc").map(|one| one.text()),
        "media_bytes": media,
    })
}

/// 往下走的时候把「经过了一个 `table:table-cell`」这个记号带下去：`Node` 没有父指针，
/// 而这一族的 frame 就住在格子里，判「在格子里」只能在下去的路上判。
/// 只收带 `draw:image` 的那些 frame —— 装图表与嵌入对象的是 `draw:object`，各有一本账
fn ods_frames(
    node: &xmlscan::Node,
    in_cell: bool,
    bytes: &[u8],
    ...
    let deeper = if node.is("table:table-cell") {
        true
    } else {
        in_cell
    };
    for one in &node.children {
        ...
        if one.is("frame") {
            if one.child("image").is_some() {
                out.push(ods_frame_picture(one, deeper, bytes));
            }
            continue;
        }
        ods_frames(one, deeper, bytes, limit, out);
    }
}

/// ODF 的表上那张图：地址只有一跳（`draw:image/@xlink:href`，**没有关系表**），
/// 所以「配不上号」这一本在这里是「压根没写 href」—— 手上这份件就有一条那样的：
/// frame 与空的 `draw:image` 都还在，href 没了，于是它既算一张图又跳不到任何字节
fn ods_pictures(host: &xmlscan::Node, bytes: &[u8], limit: usize) -> Value {
    let mut list: Vec<Value> = Vec::new();
    ods_frames(host, false, bytes, limit, &mut list);
    let frames = host.descendants("frame");
    let total = frames
        .iter()
        .filter(|one| one.child("image").is_some())
        .count();
    let others = frames.len() - total;
    let unresolved = list.iter().filter(|one| one["href"].is_null()).count();
    let missing = list
        .iter()
        .filter(|one| !one["href"].is_null() && one["media_bytes"].is_null())
        .count();
    picture_ledger(None, total, list, unresolved, missing, others)
}
...
/// 方格：行数是最大行号 + 1、列数是最大列号 + 1，洞就是空格（尾部不裁 —— 空格子也是格子）
///
/// CSV 与 markdown 两个出口共用这一份，于是那两本账的 `rows` / `columns` 是同一个数 ——
/// 各自算一遍就会有一个地方悄悄把某一列丢掉
fn square(cells: &[(usize, usize, String)]) -> (Vec<Vec<String>>, usize, usize) {
...
fn render_csv(cells: &[(usize, usize, String)]) -> (String, usize, usize) {
    let (grid, rows, cols) = square(cells);
...
/// 一格里进 markdown 该写什么：竖线要躲（`\|`），换行换成 `<br>` —— 与文档那一族的表格
/// 是同一条铺法，两个出口读同一份方格、各自按自己那族的规矩转义
fn md_cell(raw: &str) -> String {
    raw.replace('|', "\\|").replace("\n", "<br>")
}

fn render_markdown(cells: &[(usize, usize, String)]) -> (String, usize, usize) {
    let (grid, rows, cols) = square(cells);
    let mut out = String::new();
...
/// `--markdown` 的那份账：与 `--csv` 同一张方格、同一个 `--sheet` 取处，
/// 所以两本账的 `rows` / `columns` 必须相等（这条等式在探针里钉着）。
/// `separator_after_row` 说的是那行 `| --- |` 加在第 0 行之后 —— markdown 的表头是格式要求的，
/// 不是文件说「这一行是表头」；没开这个开关就没有这个键
fn markdown_report(
    want: bool,
    names: &[String],
    grids: &[Vec<(usize, usize, String)>],
    ...
    let (text, rows, columns) =
        render_markdown(grids.get(index).map(|one| &one[..]).unwrap_or_default());
    json!({
        "sheet": names[index],
        "index": index,
        "rows": rows,
        ...
        "cells_skipped": skipped,
        "line_end": "LF",
        "separator_after_row": if rows > 0 { json!(0) } else { Value::Null },
        "text": text,
    })
}
...
mod tests {
    ...
    fn run_csv(name: &str, sheet: &str) -> Value {
        render(name, sheet, true, false)
    }

    /// 只开 `--markdown`
    fn run_md(name: &str, sheet: &str) -> Value {
        render(name, sheet, false, true)
    }

    /// 两个开关一起开：比的正是「同一张方格、两个出口」那两个数相不相等
    fn run_both(name: &str, sheet: &str) -> Value {
        render(name, sheet, true, true)
    }

    fn render(name: &str, sheet: &str, csv: bool, markdown: bool) -> Value {
        let app = OfficeSheet {
            path: PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("tests/fixtures/office")
                .join(name),
    ...
    /// `--markdown`：同一张方格的第二个出口。全文期望逐字来自 `office_reader.py` 的
    /// `md_render`。pipes* 三副是这一批新做的 —— 库里原本没有一格带竖线，
    /// 那支转义没人测过就不能说它成立（换行那一支 `rich*` 早已有件，顺带钉在 ods 这边）
    #[test]
    fn markdown_renders_the_same_square_the_csv_path_fills() {
        // 不开这个开关就没有这一格：交一份空表装作有，是另一回事
        let plain = run("pipes.xlsx");
        assert!(plain["markdown"].is_null(), "{plain}");
        assert!(plain.get("markdown").is_some(), "键要在，但值是 null");

        let pipes = run_md("pipes.xlsx", "");
        let md = &pipes["markdown"];
        assert_eq!(md["sheet"], json!("格"));
        assert_eq!(md["index"], 0);
        assert_eq!(md["rows"], 5);
        assert_eq!(md["columns"], 2);
        assert_eq!(md["line_end"], "LF");
        assert_eq!(md["cells_skipped"], 0);
        assert_eq!(md["separator_after_row"], 0);
        assert_eq!(md["text"], json!("| 名称 | 备注 |\n| --- | --- |\n| a\\|b | 1\\|2\\|3 |\n| 第一行<br>带\\|竖线<br>第三行 | 两行<br>第二行 |\n| \\|首尾都带\\| | \\| |\n| 普通一格 |  |\n"));
        // 竖线躲过、换行铺平：这两个东西 markdown 的表格装不下原样
        assert!(
            md["text"].as_str().unwrap_or_default().contains("a\\|b"),
            "{md}"
        );
        assert!(
            md["text"]
        ...
        // 三个生产者铺出来的 markdown 逐字相同（ODF 与 OOXML 两套账到这一格合上）
        for name in ["pipes.ods", "pipes-lo.xlsx"] {
            assert_eq!(
                run_md(name, "")["markdown"]["text"],
                md["text"],
                "另一家的同一张方格：{name}"
            );
        }
        // 一格只有一行字时那行分隔线照加 —— 它是格式要求的，不是文件说的表头
        assert_eq!(
            run_md("book.ods", "草稿")["markdown"]["text"],
            json!("| 隐藏的草稿表 |\n| --- |\n")
        );
        ...
        assert_eq!(run_md("rich.ods", "")["markdown"]["rows"], 7);
        assert_eq!(run_md("rich.ods", "")["markdown"]["columns"], 2);

        // 两个开关一起开：比的正是「同源那张方格」那两个数不许各算一遍
        let both = run_both("pipes.xlsx", "");
        assert_eq!(both["markdown"]["rows"], both["csv"]["rows"], "{both}");
        assert_eq!(
            both["markdown"]["columns"], both["csv"]["columns"],
            "{both}"
        );
        assert_eq!(both["markdown"]["text"], md["text"], "开关搭伙不该改正文");
        assert_eq!(
            both["csv"]["text"],
            run_csv("pipes.xlsx", "")["csv"]["text"]
        );
        // 认错表名：两本账各说一句，且都把候选列出来
        let bad = run_md("book.ods", "没这张");
        assert!(bad["markdown"]["error"].is_string(), "{bad}");
    }
    ...
    fn a_shared_formula_leaves_seven_cells_with_no_text() {
        ...
        assert_eq!(
            ...
            json!({
                "formula": {
                    "of:=[.A1]*2": 1, "of:=SUM([.$A$1:.A1])": 1,
                    "of:=[.A2]*2": 1, "of:=SUM([.$A$1:.A2])": 1,
                    "of:=[.A3]*2": 1, "of:=SUM([.$A$1:.A3])": 1,
                    "of:=[.A4]*2": 1, "of:=SUM([.$A$1:.A4])": 1,
                    "of:=[.A5]*2": 1, "of:=SUM([.$A$1:.A5])": 1,
                    "of:=[.A6]*2": 1, "of:=SUM([.$A$1:.A6])": 1,
                    "of:=[.A7]*2": 1, "of:=SUM([.$A$1:.A7])": 1,
                    "of:=[.A8]*2": 1, "of:=SUM([.$A$1:.A8])": 1
                },
                "value-type": {"float": 16},
                "value": {"6": 1, "3": 8, "0": 7}
            })
        ...
        assert!(side.get("sheets_seen").is_none());
        let cells = side["cells"].as_array().expect("是数组");
        // 带公式的是格子自己，所以 `attrs` 是那一格写着的属性全表；`sheet` 是它所在那张
        // `table:table` 的名字（不是样式名），而这一族不写格子地址 —— `cell` 逐条 null
        assert_eq!(cells[0]["sheet"], "表一");
        assert_eq!(cells[0]["cell"], Value::Null);
        assert_eq!(cells[0]["style"], Value::Null);
        assert_eq!(
            cells[0]["attrs"],
            json!({"formula": "of:=[.A1]*2", "value-type": "float", "value": "6"})
        );
        ...
        assert_eq!(run("book.ods")["formula_elems"]["tables_seen"], 3);
        assert_eq!(
            run("book.ods")["formula_elems"]["cells"][0]["sheet"],
            "预算表"
        );
        // 一张公式都没有的那一份：`attr_values` 与 `formula_prefixes` 都交空表（数过了没有）
        let quiet = run("rich.ods");
        assert_eq!(quiet["formula_elems"]["formula_elems"], 0);
        assert_eq!(quiet["formula_elems"]["attr_values"], json!({}));
        assert_eq!(quiet["formula_elems"]["formula_prefixes"], json!({}));
        assert_eq!(quiet["formula_elems"]["attrs_seen"], json!([]));
        // 遗留 .xls 不交这个键（那一族的公式在 BIFF 记录树里，是另一问）
        assert!(run("book.xls")["formula_elems"].is_null());
    }

    /// 格子上的链接有四种存法，外加第五种「压根不写链接元素」的 `HYPERLINK(...)` 公式：
    /// * xlsx：`<hyperlink>` 写一个关系号，地址在表自己的 `.rels` 里（`hop=rels`）
    /// * xlsx：站内跳法不写号，地址直接写在元素的 `location` 上（`hop=inline`）
    /// * .ods：地址挂在段里那个字上（`text:a/@xlink:href`），没有第二跳也没有开关
    /// * .xls：一条 0x01B8 记录，两条分支看第二个 GUID 在不在（外部数字节、站内数码元）
    ///
    /// 四本账不并成一个形状，但同一批字必须数得出同一个数；两个 xlsx 生产者除外——
    /// 它们写的属性面本来就不同（openpyxl 不写 display，LibreOffice 每条都补一个）
    #[test]
    fn cell_links_are_ledgered_whichever_way_they_are_written() {
        let openpyxl = run("cell-links.xlsx");
        let rewritten = run("cell-links-lo.xlsx");
        assert_eq!(
            openpyxl["sheets"][0]["links"]["family"], "rels",
            "{openpyxl}"
        );
        assert_eq!(openpyxl["sheets"][0]["links"]["total"], 6, "{openpyxl}");
        assert_eq!(openpyxl["sheets"][0]["links"]["external"], 4, "{openpyxl}");
        assert_eq!(openpyxl["sheets"][0]["links"]["with_id"], 5, "{openpyxl}");
        assert_eq!(
            openpyxl["sheets"][0]["links"]["with_location"], 1,
            "{openpyxl}"
        );
        // 一个号写了、关系表里那条被删掉：target 与 location 都没有 → 指不到
        assert_eq!(
            openpyxl["sheets"][0]["links"]["unresolved"], 1,
            "{openpyxl}"
        );
        assert_eq!(
            openpyxl["sheets"][0]["links"]["with_display"], 0,
            "{openpyxl}"
        );
        assert_eq!(
            openpyxl["sheets"][0]["links"]["with_tooltip"], 1,
            "{openpyxl}"
        );
        assert_eq!(rewritten["sheets"][0]["links"]["total"], 5, "{rewritten}");
        assert_eq!(
            rewritten["sheets"][0]["links"]["with_display"], 5,
            "{rewritten}"
        );
        assert_eq!(
            rewritten["sheets"][0]["links"]["unresolved"], 0,
            "{rewritten}"
        );
        // 同一批字，两个生产者差的这一条不是读丢了，是文件自己写的差别
        assert_eq!(openpyxl["workbook"]["totals"]["links"], 7, "{openpyxl}");
        assert_eq!(rewritten["workbook"]["totals"]["links"], 6, "{rewritten}");
        for one in [&openpyxl, &rewritten] {
            assert_eq!(one["workbook"]["totals"]["hyperlink_formulas"], 1, "{one}");
            // 站内那一本在第二张表上：一条 location，没有号
            assert_eq!(one["sheets"][1]["links"]["total"], 1, "{one}");
            assert_eq!(one["sheets"][1]["links"]["internal"], 1, "{one}");
            assert_eq!(one["sheets"][1]["links"]["formula_cells"], 0, "{one}");
        }

        let ods = run("cell-links.ods");
        assert_eq!(ods["sheets"][0]["links"]["family"], "inline", "{ods}");
        assert_eq!(ods["sheets"][0]["links"]["total"], 5, "{ods}");
        // 这一族把链接全写在段里的字上，一条 `table:hyperlink` 都不写
        assert_eq!(ods["sheets"][0]["links"]["via_text_a"], 5, "{ods}");
        assert_eq!(ods["sheets"][0]["links"]["via_table_hyperlink"], 0, "{ods}");
        // 没有 TargetMode 那个开关：站外只按认得出的 scheme 数，站内是 `#` 开头那一条
        assert_eq!(ods["sheets"][0]["links"]["external"], 4, "{ods}");
        assert_eq!(ods["sheets"][0]["links"]["internal"], 1, "{ods}");
        assert_eq!(ods["sheets"][0]["links"]["formula_cells"], 1, "{ods}");
        assert_eq!(ods["workbook"]["totals"]["links"], 6, "{ods}");
        assert_eq!(ods["workbook"]["totals"]["hyperlink_formulas"], 1, "{ods}");

        let xlsbook = run("cell-links.xls");
        assert_eq!(
            xlsbook["sheets"][0]["links"]["family"], "record",
            "{xlsbook}"
        );
        assert_eq!(xlsbook["sheets"][0]["links"]["total"], 5, "{xlsbook}");
        assert_eq!(xlsbook["sheets"][0]["links"]["external"], 4, "{xlsbook}");
        // 记录条数与解出来的条数是两本账，这一份件里两者相等（每条都解得动）
        assert_eq!(xlsbook["sheets"][0]["links"]["records"], 5, "{xlsbook}");
        assert_eq!(xlsbook["sheets"][0]["links"]["whole"], 5, "{xlsbook}");
        assert!(
            xlsbook["sheets"][0]["links"]["formula_cells"].is_null(),
            "{xlsbook}"
        );
        assert_eq!(xlsbook["workbook"]["totals"]["links"], 6, "{xlsbook}");
        assert_eq!(
            xlsbook["workbook"]["totals"]["link_records"], 6,
            "{xlsbook}"
        );
        // 0x01B7 自报的那个数按写的交（实测 0，而流里有六条 0x01B8）
        assert_eq!(
            xlsbook["workbook"]["links_written"],
            json!([0]),
            "{xlsbook}"
        );

        // 同一格 C3 的站内地址：三种写法各交各的，不替它们归一
        let at = |book: &Value, sheet: usize, want: &str| -> Value {
            book["sheets"][sheet]["links"]["list"]
                .as_array()
                .expect("是数组")
                .iter()
                .find(|one| one["ref"] == json!(want))
                .cloned()
                .unwrap_or(Value::Null)
        };
        assert_eq!(
            at(&openpyxl, 0, "C3")["location"],
            "'数据'!A1",
            "{openpyxl}"
        );
        assert_eq!(at(&openpyxl, 0, "C3")["hop"], "inline", "{openpyxl}");
        // ODF 用点分隔，OOXML 与 .xls 用叹号
        assert_eq!(at(&ods, 0, "C3")["target"], "#'数据'.A1", "{ods}");
        assert_eq!(at(&xlsbook, 0, "C3")["location"], "'数据'!A1", "{xlsbook}");
        assert_eq!(at(&xlsbook, 0, "C3")["display"], "跳到数据页", "{xlsbook}");
        // .xls 没有「关系号」与「悬浮提示」这两个字段，交 null 而不是空串
        assert!(at(&xlsbook, 0, "A1")["id"].is_null(), "{xlsbook}");
        assert!(at(&xlsbook, 0, "A1")["tooltip"].is_null(), "{xlsbook}");
        assert_eq!(
            at(&xlsbook, 0, "A1")["target"],
        ...
        for name in [
            ...
            assert_eq!(
                run(name)["workbook"]["totals"]["links"],
                0,
                "{name} 不该有链接账"
            );
        }
    ...
    /// 表上那张位图那一份账：OOXML 跳三跳、ODF 只有一跳，两个生产者把同样四张源图片
    /// 写成两套明细（期望值全抄 `office_reader.py` 的对账输出，不是猜的）
    #[test]
    fn pictures_on_the_sheet_are_ledgered_per_anchor() {
        let openpyxl = run("sheet-pictures.xlsx");
        let first = openpyxl["sheets"][0]["pictures"].clone();
        let rows = first["list"].as_array().expect("是数组");
        assert_eq!(first["drawings"], 1, "{first}");
        assert_eq!(first["total"], 5);
        // 去重按这一条自己写的那跳：rId5 那条没有地址，不进这一本
        assert_eq!(first["distinct_media"], 4);
        assert_eq!(first["unresolved"], 1, "那条关系被删掉了，锚块还在");
        assert_eq!(first["missing_media"], 0);
        assert_eq!(first["other_anchors"], 0);
        assert_eq!(first["listed"], 5);
        assert_eq!(first["cut"], json!(false));
        assert_eq!(openpyxl["workbook"]["totals"]["pictures"], 7, "{openpyxl}");
        // 摆法：三种锚块元素名按文档顺序整串交，而 openpyxl 一个锚块属性都不写
        let placed: Vec<&str> = rows
            .iter()
            .map(|one| one["placed"].as_str().unwrap_or_default())
            .collect();
        assert_eq!(
            placed,
            vec![
            ...
            "跨格一条、单格三条、绝对一条，按文件里的先后"
        );
        assert_eq!(rows[0]["anchor_attrs"], json!({}), "区别不在属性上");
        assert_eq!(rows[0]["from"]["col"].as_f64(), Some(5.0));
        assert_eq!(rows[0]["from"]["col_off"].as_f64(), Some(0.0));
        assert_eq!(rows[0]["to"]["col_off"].as_f64(), Some(95250.0));
        assert_eq!(rows[0]["to"]["row_off"].as_f64(), Some(66675.0));
        // 尺寸这一家写在锚块的 ext 上，而 spPr/xfrm 整个没有
        assert_eq!(rows[1]["ext"]["cx"], "381000");
        assert!(rows[1]["xfrm"].is_null(), "openpyxl 不写 xfrm");
        assert_eq!(rows[4]["pos"]["x"], "2857500");
        assert!(rows[4]["from"].is_null(), "absoluteAnchor 两个角都不写");
        assert!(rows[4]["to"].is_null());
        assert_eq!(rows[2]["shape_name"], "第二张", "改过的那一条两家都留着");
        assert_eq!(rows[2]["descr"], "一个蓝点");
        assert_eq!(
            rows[0]["blip_attrs"],
            json!({"cstate": "print", "embed": "rId3"})
        );
        assert_eq!(rows[0]["target"], "xl/media/image3.png");
        assert_eq!(rows[0]["media_bytes"], 117, "按中央目录自报的长度");
        // 那一条断了的关系：号还在，地址与字节都不编
        assert_eq!(rows[3]["blip_id"], "rId5");
        assert!(rows[3]["target"].is_null(), "{rows:?}");
        assert!(rows[3]["media_bytes"].is_null());
        // 每张表有自己那一份画法账：第四张表一个都没挂
        assert_eq!(openpyxl["sheets"][1]["pictures"]["drawings"], 1);
        assert_eq!(
            openpyxl["sheets"][1]["pictures"]["list"][0]["drawing"],
            "xl/drawings/drawing2.xml"
        );
        assert_eq!(openpyxl["sheets"][3]["pictures"]["drawings"], 0);
        assert_eq!(openpyxl["sheets"][3]["pictures"]["total"], 0);

        let rewritten = run("sheet-pictures-lo.xlsx");
        let lo = rewritten["sheets"][0]["pictures"].clone();
        let lo_rows = lo["list"].as_array().expect("是数组");
        assert_eq!(lo["total"], 4);
        assert_eq!(lo["distinct_media"], 3, "同一张 image1.png 被两个锚块引用");
        assert_eq!(lo["unresolved"], 0);
        assert_eq!(
            rewritten["workbook"]["totals"]["pictures"], 6,
            "{rewritten}"
        );
        // 同一家族里第二个生产者把区别从元素名搬进了属性名：只看元素名会读成一种摆法
        assert!(lo_rows
            .iter()
            .all(|one| one["placed"] == json!("twoCellAnchor")));
        assert_eq!(lo_rows[0]["anchor_attrs"]["editAs"], "twoCell");
        assert_eq!(lo_rows[1]["anchor_attrs"]["editAs"], "oneCell");
        assert_eq!(lo_rows[3]["anchor_attrs"]["editAs"], "absolute");
        // 而尺寸这一家在 spPr/xfrm 上：锚块级的 ext 整个没有
        assert!(lo_rows[0]["ext"].is_null());
        assert_eq!(lo_rows[0]["xfrm"]["off"]["x"], "3009960");
        assert_eq!(lo_rows[0]["xfrm"]["ext"]["cx"], "1900800");
        assert_eq!(lo_rows[0]["xfrm"]["attrs"], json!({}));
        // 两条换算不等：同一个「跨三格」在两家是两组 EMU，谁也不替谁归一
        assert_eq!(lo_rows[0]["to"]["col_off"].as_f64(), Some(95040.0));
        assert_eq!(
            lo_rows[0]["blip_attrs"],
            json!({"embed": "rId1"}),
            "没有 cstate 那一样"
        );
        assert_eq!(lo_rows[3]["target"], "xl/media/image3.jpeg");
        assert_eq!(lo_rows[3]["media_bytes"], 661);

        let ods = run("sheet-pictures.ods");
        let frames = ods["sheets"][0]["pictures"].clone();
        let frames_rows = frames["list"].as_array().expect("是数组");
        // 这一族没有画法部件那一层：交 null 不交 0
        assert!(frames["drawings"].is_null(), "{frames}");
        assert_eq!(frames["total"], 5);
        assert_eq!(frames["distinct_media"], 3);
        assert_eq!(frames["unresolved"], 1, "压根没写 href 的那一条");
        assert_eq!(ods["workbook"]["totals"]["pictures"], 7, "{ods}");
        assert_eq!(frames_rows[0]["in_cell"], json!(false));
        assert_eq!(
            frames_rows[1]["in_cell"],
            json!(true),
            "这一族的图住在格子里"
        );
        // 尺寸是自带单位的串，原样交不换算
        assert_eq!(frames_rows[0]["x"], "7.938cm");
        assert_eq!(frames_rows[0]["width"], "1.587cm");
        assert_eq!(
            frames_rows[3]["end_cell_address"], "图与格.I9",
            "只有跨格那一条写了"
        );
        assert!(frames_rows[0]["end_cell_address"].is_null());
        assert_eq!(
            frames_rows[0]["href"],
            "Pictures/100000000000003C0000001E5F3C57E6.jpg"
        );
        assert_eq!(frames_rows[0]["media_bytes"], 661);
        assert_eq!(frames_rows[2]["alt"], "一个蓝点");
        // 断了的那一条：frame 与空的 draw:image 都还在，只是没有地址
        assert_eq!(frames_rows[4]["name"], "Image 5");
        assert!(frames_rows[4]["href"].is_null());
        assert_eq!(frames_rows[4]["image_written"], json!({}));
        assert_eq!(ods["sheets"][2]["name"], "藏着");
        assert_eq!(ods["sheets"][2]["pictures"]["total"], 1, "看不见不等于没有");
        assert_eq!(ods["sheets"][3]["pictures"]["total"], 0);

        // 画法部件里那些不是位图的摆位（图表走 graphicFrame）：另记一本，不算坏图
        for name in ["chart.xlsx", "chart-lo.xlsx"] {
            let book = run(name);
            assert_eq!(book["sheets"][0]["pictures"]["other_anchors"], 2, "{name}");
            assert_eq!(book["sheets"][0]["pictures"]["total"], 0);
            assert_eq!(
                book["sheets"][0]["pictures"]["unresolved"], 0,
                "不是两张坏掉的图"
            );
            assert_eq!(
                book["sheets"][0]["pictures"]["drawings"], 1,
                "画法部件确实在"
            );
        }
        // ODF 那一族的图表带一张预览缓存位图：它确实是包里的一张图，开关跟着交
        let odf_chart = run("chart.ods");
        assert_eq!(odf_chart["sheets"][0]["pictures"]["total"], 2);
        assert_eq!(
            odf_chart["sheets"][0]["pictures"]["list"][0]["also_object"],
            json!(true)
        );
        assert_eq!(
            odf_chart["sheets"][0]["pictures"]["list"][1]["also_object"],
            json!(true)
        );
        assert_eq!(
            odf_chart["sheets"][0]["pictures"]["list"][0]["href"],
            "ObjectReplacements/Object 1"
        );

        // 第四族：字节只住在 global 流的那条 OfficeArt 记录里，表子流一个字节都没有
        let xlsbook = run("sheet-pictures.xls");
        let blips = xlsbook["workbook"]["pictures"]["blips"]
            .as_array()
            .expect("是数组");
        assert_eq!(xlsbook["workbook"]["pictures"]["total"], 3);
        assert_eq!(blips.len(), 3);
        assert_eq!(blips[0]["offset"], 1130);
        assert_eq!(blips[0]["instance"], 6);
        assert_eq!(blips[0]["cb"], 178);
        assert_eq!(blips[0]["magic_at"], 61);
        assert_eq!(blips[0]["kind"], "png");
        assert_eq!(blips[2]["instance"], 5, "这一族给 JPEG 写的号不一样");
        assert_eq!(blips[2]["kind"], "jpeg");
        // 跨一族的自证：同一批源图片在四族里的字节数是同一批数
        assert_eq!(blips[0]["inline_bytes"], 117);
        assert_eq!(blips[1]["inline_bytes"], 110);
        assert_eq!(blips[2]["inline_bytes"], 661);
        assert_eq!(
            xlsbook["workbook"]["pictures"]["drawing_groups"][0]["offset"],
            1054
        );
        assert_eq!(xlsbook["sheets"][0]["pictures"]["shapes"], 5);
        assert_eq!(xlsbook["sheets"][0]["pictures"]["drawing_records"], 5);
        assert_eq!(xlsbook["sheets"][3]["pictures"]["shapes"], 0);
        assert_eq!(
            xlsbook["sheets"][3]["pictures"]["drawing_records"], 1,
            "没挂图的表也写了一条画法记录"
        );
        assert_eq!(xlsbook["workbook"]["totals"]["picture_shapes"], 7);
        assert_eq!(
            xlsbook["workbook"]["totals"]["drawing_records"], 8,
            "两本账不是一本"
        );

        // 反面对照：没有位图的件里键照样在、八个数全交（缺键与零条是两件事）
        for name in ["book.xlsx", "hidden.xlsx"] {
            for one in run(name)["sheets"].as_array().expect("是数组") {
                assert_eq!(one["pictures"].get("total"), Some(&json!(0)), "{name}");
                assert_eq!(one["pictures"]["drawings"], 0, "这一族数过了，没有");
                assert_eq!(one["pictures"]["list"].as_array().expect("是数组").len(), 0);
            }
        }
        for one in run("print-area.ods")["sheets"].as_array().expect("是数组") {
            assert_eq!(one["pictures"]["total"], 0);
            assert!(one["pictures"]["drawings"].is_null(), "这一族没有那一层");
        }
        let plain = run("book.xls");
        assert_eq!(plain["workbook"]["pictures"]["total"], 0);
        assert_eq!(plain["workbook"]["totals"]["picture_shapes"], 0);
        assert_eq!(plain["workbook"]["totals"]["drawing_records"], 3);
    }
}

...
mod tests {
    ...
    fn a_picture_has_two_sizes_two_alts_and_two_locks() {
        ...
        // RTF 只从流里数得出张数，逐张账本不交（那个形状在这族无从谈起）
        let rtf = run("images.rtf");
        assert_eq!(rtf["structure"]["pictures"], 1, "{rtf}");
        assert!(
            rtf["structure"].get("picture_list").is_none(),
            "键整个不在，不是空表：{}",
            rtf["structure"]
        );
    }

...
fn run_office_doc(app: &OfficeDoc, ctx: &Context) -> Result<Value, AppError> {
    ...
    let result = if doc.family == Family::Ooxml && doc.app == "word" {
        ...
        json!({
            ...
            // 那张纸：一个 `w:sectPr` 一张（每节自己写尺寸与边距，单位 twips 换成 0.1mm）
        ...
        let page_setup = match zipread::member(bytes, "styles.xml", DEFAULT_MEMBER_CAP) {
            Ok(member) => {
                let styles_root = xmlscan::parse_str(&member.as_text());
                crate::paper::ledger(crate::paper::odf(&styles_root, limit))
            }
            Err(why) => {
        ...
        notes.push(
            "RTF 这一支只报流里数得清的东西：段落按 par 控制字切，注按目标群分\
             ...
             与 docx 的 twips、odt 的「21.59cm」换成同一个 0.1mm 整数；\
...
mod tests {
    ...
    /// 换成 0.1mm 的整数之后要给同一个数。期望值来自 `lyco_pages.py` 的独立读取。
    ...
    fn the_same_paper_comes_out_of_three_spellings() {
        ...
        for stem in ["notes", "notes-end", "tables"] {
            ...
            for one in &got {
                assert_eq!(one["page_setup"]["unit"], "0.1mm", "{stem}：{one}");

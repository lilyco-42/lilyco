    ...
    let result = if doc.family == Family::Ooxml && doc.app == "word" {
        ...
        notes.push(
            "RTF 这一支只报流里数得清的东西：段落按 par 控制字切，注按目标群分\
             （尾注靠群里的 ftnalt 判）；样式名、表格线、批注与目录都不判，\
             所以那些项是 null —— null 是「没看」，不是「没有」"
                .to_string(),
        );
        json!({
            "path": app.path.to_string_lossy(),

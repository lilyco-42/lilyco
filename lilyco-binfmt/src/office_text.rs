...
pub struct OfficeText {
    ...
    /// 把结构搬成 markdown 一起交（目前五族走这一本：OOXML 的 word、OOXML 的 powerpoint、
    /// ODF 的 odt、ODF 的 odp 与 RTF）
...
fn run_office_text(app: &OfficeText, ctx: &Context) -> Result<Value, AppError> {
    ...
    match doc.family {
            ...
            match read(bytes, part) {
                Some(member) => {
                    ...
                    if app.markdown {
                        ...
                        markdown = Some(crate::markdown::docx(bytes, budget));
                    }
                }
        ...
        Family::Ooxml if doc.app == "powerpoint" => {
            ...
            if app.markdown {
                // 大纲那一本走放映顺序（`presentation.xml` 的 sldId），不是上面那份部件名序：
                // 两处页序本来可以不一样，这一本按页交就得按放映交
                let budget = crate::opack::take_limit(app.max_chars, MAX_CHARS_DEFAULT);
                markdown = Some(crate::markdown::pptx_deck(bytes, budget));
            }
        }
        ...
        Family::Rtf => {
            ...
            if app.markdown {
                // 第五族：段、样式、列表三本账都出自上面那一次扫描，不再扫第二遍
                let budget = crate::opack::take_limit(app.max_chars, MAX_CHARS_DEFAULT);
                markdown = Some(crate::markdown::rtf(&one, budget));
            }
            ...
            // 批注是同一个流里的第三种目标群（`{\*\annotation …}`）：字不许混进正文，
            // 出处与作者按 docx / odt 那两支的字段名交。`date` 一律 null —— 那一群自己
            // 写的 `{\*\atndate …}` 两个样本都对不上 docx 那边的 w:date，解不动就把
            // 原样那串交在 date_written 里，不替它挑一个历法。`anchor` 是文件自己的号
            // （与锚区两头 `{\*\atrfstart N}` / `{\*\atrfend N}` 同一个数）
            for had in one.annotations.iter() {
                let text = had["text"].as_str().unwrap_or_default();
                if text.is_empty() && !app.keep_empty {
                    continue;
                }
                ...
                paragraphs.push(json!({
                    ...
                    "author": had["author"],
                    "date": Value::Null,
                    "date_written": had["date_written"],
                    "anchor": had["ref"],
                }));
            }
            if !one.annotations.is_empty() {
                notes.push(format!(
                    "批注的 {} 条从 annotation 群里读出来（那一群照旧整群跳过，一个字不进正文）；\
                     作者条数 {}",
                    one.annotations.len(),
                    one.annotation_authors
                ));
            }
        }
    ...
    if app.markdown && markdown.is_none() {
        notes.push(
            "markdown 这一本目前交五族：OOXML 的 word（docx / docm）、OOXML 的 powerpoint（pptx / pptm）、ODF 的 odt、ODF 的 odp 与 RTF —— 这份件不是那五族，所以那个键整个不在（缺键 = 这一族还没搬，不是空文档）".to_string(),
        );
    }
    let mut result = json!({
    ...
    // 没开 `--markdown` 就整个键都不给（不给一份空串装作渲染过）
    if let Some(one) = markdown {
        result["markdown"] = one;
    }
...
mod tests {
    ...
    /// 放映的大纲搬进 markdown：条目标不标是文件自己说的，两家因此只差两个 `- `
    #[test]
    fn a_deck_outline_marks_bullets_only_where_the_file_said_so() {
        let hand = rendered("deck.pptx");
        let md_hand = &hand["markdown"];
        assert_eq!(md_hand["family"], "pptx");
        assert_eq!(md_hand["available"], true);
        assert_eq!(md_hand["pages"], 2);
        assert_eq!(md_hand["titles"], 2);
        assert_eq!(md_hand["titles_missing"], 0);
        assert_eq!(md_hand["paragraphs"], 2);
        assert_eq!(md_hand["headings"], 0);
        assert_eq!(md_hand["bullets_written"], 0);
        assert_eq!(md_hand["bullets_denied"], 0);
        assert_eq!(md_hand["bullets_silent"], 2);
        assert_eq!(md_hand["tables"], 1);
        assert_eq!(md_hand["table_rows"], 2);
        assert_eq!(md_hand["empty_dropped"], 0);
        assert_eq!(md_hand["notes_pages"], 1);
        assert_eq!(md_hand["links"], 0);
        assert_eq!(md_hand["pictures"], 1);
        assert_eq!(md_hand["levels_written"], 0);
        assert_eq!(md_hand["blocks"], 5);
        assert_eq!(md_hand["chars"], 84);
        assert_eq!(md_hand["cut"], false);
        assert_eq!(md_hand["text"], "# 预算评审\n\n新增两台 64 核应用服务器\n\n第二条要点\n\n# 第二页：数字\n\n| 科目 | 金额 |\n| --- | --- |\n| 服务器 | 124000 |\n");
        // LibreOffice 重写同一份稿子：给两条写了 buChar，于是那两行前面有 `- `，
        // 而列表项之间不再空行 —— 两家相差 3 个码位（87 对 84），块数不变
        let back = rendered("deck-lo.pptx");
        let md_back = &back["markdown"];
        assert_eq!(md_back["family"], "pptx");
        assert_eq!(md_back["available"], true);
        assert_eq!(md_back["pages"], 2);
        assert_eq!(md_back["titles"], 2);
        assert_eq!(md_back["titles_missing"], 0);
        assert_eq!(md_back["paragraphs"], 2);
        assert_eq!(md_back["headings"], 0);
        assert_eq!(md_back["bullets_written"], 2);
        assert_eq!(md_back["bullets_denied"], 0);
        assert_eq!(md_back["bullets_silent"], 0);
        assert_eq!(md_back["tables"], 1);
        assert_eq!(md_back["table_rows"], 2);
        assert_eq!(md_back["empty_dropped"], 0);
        assert_eq!(md_back["notes_pages"], 1);
        assert_eq!(md_back["links"], 0);
        assert_eq!(md_back["pictures"], 1);
        assert_eq!(md_back["levels_written"], 0);
        assert_eq!(md_back["blocks"], 5);
        assert_eq!(md_back["chars"], 87);
        assert_eq!(md_back["cut"], false);
        assert_eq!(md_back["text"], "# 预算评审\n\n- 新增两台 64 核应用服务器\n- 第二条要点\n\n# 第二页：数字\n\n| 科目 | 金额 |\n| --- | --- |\n| 服务器 | 124000 |\n");
        assert_eq!(md_back["bullets_silent"], 0, "那两段在这家不再是沉默的");
        assert_eq!(md_back["blocks"], md_hand["blocks"], "块数一根没多一根没少");
        // 一张表在两家手里铺成同一份 markdown（与 --csv 那条同一批件）
        let tab = rendered("deck-tables.pptx");
        let md_tab = &tab["markdown"];
        assert_eq!(md_tab["family"], "pptx");
        assert_eq!(md_tab["available"], true);
        assert_eq!(md_tab["pages"], 1);
        assert_eq!(md_tab["titles"], 1);
        assert_eq!(md_tab["titles_missing"], 0);
        assert_eq!(md_tab["paragraphs"], 0);
        assert_eq!(md_tab["headings"], 0);
        assert_eq!(md_tab["bullets_written"], 0);
        assert_eq!(md_tab["bullets_denied"], 0);
        assert_eq!(md_tab["bullets_silent"], 0);
        assert_eq!(md_tab["tables"], 1);
        assert_eq!(md_tab["table_rows"], 3);
        assert_eq!(md_tab["empty_dropped"], 0);
        assert_eq!(md_tab["notes_pages"], 0);
        assert_eq!(md_tab["links"], 0);
        assert_eq!(md_tab["pictures"], 0);
        assert_eq!(md_tab["levels_written"], 0);
        assert_eq!(md_tab["blocks"], 2);
        assert_eq!(md_tab["chars"], 95);
        assert_eq!(md_tab["cut"], false);
        assert_eq!(md_tab["text"], "# 表格那一页\n\n| 科目<br>金额 |  | 备注 |\n| --- | --- | --- |\n| 服务器 | 124000 | 含税 |\n| 网络<br>设备 | 8000 |  |\n");
        let other = rendered("deck-tables-lo.pptx");
        assert_eq!(
            other["markdown"], tab["markdown"],
            "两家那张表的账要一格不差"
        );
        // 一份没有标题形状的件：一页也不替它编一个 `#`
        let notitle = rendered("deck-tr.pptx");
        assert_eq!(notitle["markdown"]["titles"], 0);
        assert_eq!(notitle["markdown"]["titles_missing"], 3);
        assert!(
            !notitle["markdown"]["text"]
                .as_str()
                .unwrap_or_default()
                .contains("# "),
            "没有标题就不要有标题行"
    ...
    /// RTF 的批注：作者与正文分在注的**前后两格**（`{\\*\\atnauthor …}` 在前、
    /// `{\\*\\annotation …}` 在后），注自己带一个号（与锚区两头 `atrfstart` / `atrfend`
    /// 同一个数）。日期解不出来就交 null，原样在 `date_written` 里；注的字一个不落正文。
    /// 同一批字的 docx 那一份作者是「刘奇」—— 中文名字 LibreOffice 在 RTF 里写不出来，
    /// 只剩两个问号：那是**生产者的差**，按文件写的交，不替它认回来
    /// （期望值来自 `lyco_rtf.py` 的 rtf_text）
    #[test]
    fn rtf_comments_come_out_of_their_starred_groups() {
        let out = run("comments.rtf", 20000, false);
        let items = out["paragraphs"].as_array().expect("是数组");
        let side: Vec<Value> = items
            .iter()
            .filter(|one| one["from"] == "comment")
            .map(|one| {
                json!([
                    one["author"],
                    one["text"],
        ...
        assert_eq!(
            side,
            vec![
                json!(["liuqi", "这里要补上不含税口径", "0", null, "-2014723526"]),
                json!([
                    "??",
                    "这个数要找财务确认一下，第二行接着写",
                    "1",
        ...
        // 同一段字在 OOXML 那一份里作者名是完整的（两家各按各的文件写）
        let docx = run("comments.docx", 20000, false);
        let authors: Vec<&str> = docx["paragraphs"]
            .as_array()
            .expect("是数组")
            .iter()
            .filter(|one| one["from"] == "comment")
            .filter_map(|one| one["author"].as_str())
            .collect();
        assert_eq!(
            authors,
            vec!["liuqi", "刘奇"],
        ...
        let dates: Vec<&str> = docx["paragraphs"]
            .as_array()
            .expect("是数组")
            .iter()
            .filter(|one| one["from"] == "comment")
            .filter_map(|one| one["date"].as_str())
            .collect();
        assert!(
            dates.iter().all(|one| one.starts_with("2026-")),
            "docx 有 ISO 日期：{dates:?}"
        );
    }
    ...
    /// 结构搬进 markdown：两个生产者的 docx 渲染一字不差，而「列表号写在样式上还是段上」在账上分开；
    /// 同一份稿子的 odt 那副只有图片地址那一行不同。
    /// 期望值全部来自 `scripts/acceptance/lyco_markdown.py`（docx_markdown / odf_markdown）。
    #[test]
    fn the_same_draft_from_two_producers_renders_identically() {
        let hand = rendered("md.docx");
        let back = rendered("md-lo.docx");
        assert_eq!(hand["markdown"]["family"], "ooxml");
        assert_eq!(hand["markdown"]["blocks"], 18);
        assert_eq!(hand["markdown"]["paragraphs"], 10);
        assert_eq!(hand["markdown"]["headings"], 2);
        assert_eq!(hand["markdown"]["list_items"], 5);
        assert_eq!(hand["markdown"]["bullet_items"], 3);
        assert_eq!(hand["markdown"]["ordered_items"], 2);
        assert_eq!(hand["markdown"]["tables"], 1);
        assert_eq!(hand["markdown"]["table_rows"], 3);
        assert_eq!(hand["markdown"]["empty_dropped"], 2);
        assert_eq!(hand["markdown"]["chars"], 359);
        assert_eq!(hand["markdown"]["cut"], json!(false));
        // 号写在样式上 vs 抄到段上：渲染一字不差，账本却分得清
        assert_eq!(hand["markdown"]["list_from_style"], 5);
        assert_eq!(back["markdown"]["list_from_style"], 0);
        assert_eq!(hand["markdown"]["text"], back["markdown"]["text"]);
        let text = hand["markdown"]["text"].as_str().expect("是串");
        assert!(text.starts_with("# 结构：一级\n\n## 结构：二级\n"));
        assert!(text.contains("前**粗**中*斜*尾***粗斜***"));
        assert!(text.contains("竖线 | 反引号"));
        assert!(text.contains("尾格 \\| 带竖线"));
        assert!(text.contains("\n\\# 这不是标题"));
        assert!(text.contains("\n\\1. 这不是编号"));
        assert!(text.contains("- 圆点第一条\n- 圆点第二条\n- 缩进一层的那条\n1. 编号第一条"));
        assert!(text.contains("[预算制度](https://example.com/budget)"));
        assert!(text.contains("硬换行之前\\\n硬换行之后"));
        assert!(text.contains("![](media/image1.png)"));
        // 句中两个空格：docx 靠 xml:space="preserve" 存着，渲染不许缩掉
        assert_eq!(text.matches("两处空格  之间是一个记号").count(), 1);
        // 解不到编号格式的那两条：渲染挑最保守的 `- `，而 unresolved_fmt 把这件事说在账上
        let lists = rendered("lists.docx");
        assert_eq!(lists["markdown"]["list_items"], 6);
        assert_eq!(lists["markdown"]["unresolved_fmt"], 2);
        assert_eq!(lists["markdown"]["bullet_items"], 4);
        assert_eq!(lists["markdown"]["ordered_items"], 2);
        assert_eq!(lists["markdown"]["list_from_style"], 3);
        assert!(lists["markdown"]["text"]
            .as_str()
            .unwrap_or_default()
            .contains("  - 直接挂在段上的第二级"));
        // 同一份稿子的第三副样子（odt）：块数一样而记号是另一族的写法
        let odt = rendered("md.odt");
        assert_eq!(odt["markdown"]["family"], "odf");
        assert_eq!(odt["markdown"]["blocks"], 18);
        assert_eq!(odt["markdown"]["paragraphs"], 10);
        assert_eq!(odt["markdown"]["headings"], 2);
        assert_eq!(odt["markdown"]["list_items"], 5);
        assert_eq!(odt["markdown"]["tables"], 1);
        assert_eq!(odt["markdown"]["table_rows"], 3);
        assert_eq!(odt["markdown"]["empty_dropped"], 2);
        assert_eq!(odt["markdown"]["chars"], 388);
        // 号一律来自 text:list-style、字要一跳字符样式、空格记号要展开
        assert_eq!(odt["markdown"]["lists_named"], 3);
        assert_eq!(odt["markdown"]["lists_unnamed"], 0);
        assert_eq!(odt["markdown"]["spans_unresolved"], 0);
        assert_eq!(odt["markdown"]["space_markers"], 1);
        assert_eq!(odt["markdown"]["links"], 1);
        assert_eq!(odt["markdown"]["images"], 1);
        let odt_text = odt["markdown"]["text"].as_str().expect("是串");
        // 跨族同形：35 行里只有图片地址那一行不同（各按自己文件写的交）
        assert_eq!(text.lines().count(), 35);
        assert_eq!(odt_text.lines().count(), 35);
        let apart: Vec<usize> = text
            .lines()
            .zip(odt_text.lines())
            .enumerate()
            .filter(|(_, (a, b))| a != b)
            .map(|(i, _)| i)
            .collect();
        assert_eq!(apart, vec![32]);
        assert!(odt_text.contains("![](Pictures/1000000100000008000000088E4DF5D4.png)"));
        assert_eq!(odt_text.matches("两处空格  之间是一个记号").count(), 1);
        // 嵌在正文段里的批注与注：字不进正文，跳过的条数在账上
        let ann = rendered("notes.odt");
        assert_eq!(ann["markdown"]["annotations_dropped"], 1);
        assert_eq!(ann["markdown"]["notes_dropped"], 0);
        assert_eq!(
            ann["markdown"]["text"]
                .as_str()
                .unwrap_or_default()
        ...
        let ends = rendered("notes-end.odt");
        assert_eq!(ends["markdown"]["notes_dropped"], 3);
        assert_eq!(ends["markdown"]["annotations_dropped"], 0);
        let ends_text = ends["markdown"]["text"].as_str().expect("是串");
        assert!(ends_text.contains("carries a footnote"));
        assert_eq!(
            ends_text
                .matches("Footnote: the numbers are gross.")
                .count(),
            0
        );
        assert_eq!(ends_text.matches("Endnote: the totals").count(), 0);
        // 一跳字符样式落没落到字上，styled-text 那份最直接的凭据
        let sty = rendered("styled-text.odt");
        assert_eq!(sty["markdown"]["blocks"], 15);
        assert_eq!(sty["markdown"]["paragraphs"], 15);
        assert_eq!(sty["markdown"]["chars"], 239);
        assert!(sty["markdown"]["text"]
            .as_str()
            .unwrap_or_default()
            .contains("这一段只点加粗：**甲乙丙**"));
        // 这一族还没搬的那一份：键整个不在（缺键 = 这一族没看，不是空文档）；
        // 没开开关时也一样不给。RTF 现在交了，所以这条凭据挪到遗留 .doc 上
        assert!(rendered("notes.doc")["markdown"].is_null());
        assert!(run("md.docx", 20000, false)["markdown"].is_null());
    }

    /// RTF → markdown（第五族）。整本账逐字来自 `scripts/acceptance/lyco_rtf.py` 的
    /// `rtf_markdown`；三条规矩各钉一处：标签是文件自己写也要先摘的（`labels_stripped`）、
    /// 圆点还是编号看那一级自己写的 `levelnfc`、表在这一族摊平成一段带制表记号的字
    /// 所以 `tables` 是 null 而不是 0
    #[test]
    fn rtf_is_the_fifth_markdown_family() {
        for (name, want) in [
            (
                "charstyles.rtf",
                json!({"family": "rtf", "available": true, "text": "只有样式说的：强调的字\n\n样式说斜、段上自己说粗：又粗又斜\n\n样式里还写着颜色与大写：淡淡的\n\n这一段什么样式都不点\n", "chars": 59, "cut": false, "blocks": 4, "tables": null, "paragraphs": 4, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "comments.rtf",
                json!({"family": "rtf", "available": true, "text": "第一段：不含税口径\n\n第二段：金额待确认\n\n第三段：这一段没有批注\n", "chars": 34, "cut": false, "blocks": 3, "tables": null, "paragraphs": 3, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "fields.rtf",
                json!({"family": "rtf", "available": true, "text": "# 域与跳转\n\n题注：1\n\n跳到那张表跳一个坏了的名（站内跳转，不占关系表；第二条点的是一个不存在的名）\n\n被内部链接指着的那一段\n\n自动日期：2026-09-25\n\n1（页码写在正文里一次，页脚里一次）\n", "chars": 103, "cut": false, "blocks": 6, "tables": null, "paragraphs": 6, "headings": 1, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "images.rtf",
                json!({"family": "rtf", "available": true, "text": "图前的一段。\n\n图后的一段。\n", "chars": 15, "cut": false, "blocks": 2, "tables": null, "paragraphs": 2, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 1, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "lists.rtf",
                json!({"family": "rtf", "available": true, "text": "这一段不在列表里\n\n1. 编号列表第一项\n1. 编号列表第二项\n- 圆点列表第一项\n- 直接挂在段上的第一级\n  1. 直接挂在段上的第二级\n\n点了一个不存在的编号\n", "chars": 83, "cut": false, "blocks": 7, "tables": null, "paragraphs": 7, "headings": 0, "list_items": 5, "bullet_items": 2, "ordered_items": 3, "items_unlabelled": 0, "labels_stripped": 5, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 5}),
            ),
            (
                "notes-end.rtf",
                json!({"family": "rtf", "available": true, "text": "Quarterly budget note.\n\nThis sentence carries a footnote and keeps going.\n\nA second line carries a second note\n\nLast line.\n", "chars": 123, "cut": false, "blocks": 4, "tables": null, "paragraphs": 4, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "notes-hf.rtf",
                json!({"family": "rtf", "available": true, "text": "# 带页眉的一页\n\n正文只有一句：这一份是用来测页眉页脚那条分支的。\n\n换节之后的一段正文。\n", "chars": 47, "cut": false, "blocks": 3, "tables": null, "paragraphs": 3, "headings": 1, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 2, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "notes.rtf",
                json!({"family": "rtf", "available": true, "text": "# 一级标题：预算口径\n\n第三季度服务器预算为十二万四千元\n\n## 二级标题：明细\n\n科目 金额\n\n服务器 124000\n\n口径见 预算制度\n\n最后一页说明：数字为含税口径\n", "chars": 87, "cut": false, "blocks": 7, "tables": null, "paragraphs": 7, "headings": 2, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 2, "styles_unresolved": 0, "tabs": 2}),
            ),
            (
                "paper-a4.rtf",
                json!({"family": "rtf", "available": true, "text": "# A4 纵向的这一节\n\n这一节的纸是 210×297，边距 2 厘米与 3 厘米。\n\n# 横过来的那一节\n\n同一份文件里两节，纸的宽高对调，边距也不同。\n", "chars": 78, "cut": false, "blocks": 4, "tables": null, "paragraphs": 4, "headings": 2, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 1, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "para.rtf",
                json!({"family": "rtf", "available": true, "text": "第一段：两端对齐，左缩进三厘米，首行缩进两个字\n\n第二段：右对齐，悬挂缩进，固定行距 18 磅\n\n第三段：什么都不设\n\n第四段：按字数缩进\n\n这一节排在两栏里\n", "chars": 80, "cut": false, "blocks": 5, "tables": null, "paragraphs": 5, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "styled-text.rtf",
                json!({"family": "rtf", "available": true, "text": "基准段：什么都不点。\n\n这一段只点加粗：甲乙丙\n\n这一段只点斜体：甲乙丙\n\n这一段只点下划线：甲乙丙\n\n这一段只点删除线：甲乙丙\n\n这一段只点上标：甲乙丙\n\n这一段只点红色：甲乙丙\n\n这一段只点高亮：甲乙丙\n\n这一段只点小一号：甲乙丙\n\n这一段只点换字体：甲乙丙\n\n这一段明确写着不粗：甲乙丙\n\n这一段明确写着不斜：甲乙丙\n\n一串字里两个孩子：甲乙丙\n\n一段里三种字：这一串点粗。这一串什么都不点。这一串又点斜。\n\n尾部一段：一个字都不点。\n", "chars": 221, "cut": false, "blocks": 15, "tables": null, "paragraphs": 15, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 0}),
            ),
            (
                "tables.rtf",
                json!({"family": "rtf", "available": true, "text": "# 两张表的样本\n\n中间一段普通话\n\nR0C0 R0C1\n\nR1C0 R1C1\n\nR2C0 R2C1\n\n两张表之间的一段\n\nR0C0 R0C1\n\nR1C0 R1C1\n\n两张表之间的一段\n\n## 二级标题\n", "chars": 102, "cut": false, "blocks": 10, "tables": null, "paragraphs": 10, "headings": 2, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 5}),
            ),
            (
                "tabs.rtf",
                json!({"family": "rtf", "available": true, "text": "制表位那份账\n\n左对齐无引导 第一段字 右边\n\n右对齐点引导 第一段字 右边\n\n居中长划引导 第一段字 右边\n\n小数点对齐 第一段字 右边\n", "chars": 70, "cut": false, "blocks": 5, "tables": null, "paragraphs": 5, "headings": 0, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 0, "styles_unresolved": 0, "tabs": 4}),
            ),
            (
                "toc-full.rtf",
                json!({"family": "rtf", "available": true, "text": "目录\n\n结构：一级 1\n\n结构：二级 2\n\n# 结构：一级\n\n## 结构：二级\n\n普通话一段：没有形状\n\n前粗中斜尾粗斜\n\n记号 \\*下\\_划线 \\[方括号\\] \\<尖\\> 反斜杠 \\\\ 竖线 | 反引号 \\`\n\n两处空格  之间是一个记号\n\n\\# 这不是标题，这是文件里写着的字\n\n\\1. 这不是编号，这也是文件里写着的字\n\n- 圆点第一条\n- 圆点第二条\n- 缩进一层的那条\n1. 编号第一条\n1. 编号第二条\n\n科目 金额\n\n服务器 \\* 两台 124000\n\n这一格有\n\n两段字 尾格 | 带竖线\n\n口径见 预算制度\n\n硬换行之前\n\n硬换行之后\n\n最后一页\n", "chars": 283, "cut": false, "blocks": 24, "tables": null, "paragraphs": 24, "headings": 2, "list_items": 5, "bullet_items": 3, "ordered_items": 2, "items_unlabelled": 0, "labels_stripped": 5, "empty_dropped": 4, "styles_unresolved": 0, "tabs": 10}),
            ),
            (
                "toc.rtf",
                json!({"family": "rtf", "available": true, "text": "目录\n\n一级标题：预算口径\n\n# 一级标题：预算口径\n\n第三季度服务器预算为十二万四千元\n\n## 二级标题：明细\n\n科目 金额\n\n服务器 124000\n\n口径见 预算制度\n\n最后一页说明：数字为含税口径\n", "chars": 102, "cut": false, "blocks": 9, "tables": null, "paragraphs": 9, "headings": 2, "list_items": 0, "bullet_items": 0, "ordered_items": 0, "items_unlabelled": 0, "labels_stripped": 0, "empty_dropped": 2, "styles_unresolved": 0, "tabs": 2}),
            ),
        ] {
        ...
        assert_eq!(lists["markdown"]["list_items"], 5, "{lists}");
        assert_eq!(lists["markdown"]["bullet_items"], 2);
        assert_eq!(lists["markdown"]["ordered_items"], 3);
        assert_eq!(lists["markdown"]["labels_stripped"], 5);
        assert_eq!(
            lists["markdown"]["tables"],
            Value::Null,
            "这一族没有表格结构"
        );
        let tables = rendered("tables.rtf");
        assert_eq!(tables["markdown"]["tabs"], 5, "{tables}");
        assert_eq!(tables["markdown"]["list_items"], 0);
        let notes = rendered("notes.rtf");
        assert_eq!(notes["markdown"]["paragraphs"], 7, "{notes}");
        assert_eq!(notes["markdown"]["headings"], 2);
    }
}

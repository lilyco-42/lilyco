...
mod tests {
    ...
    fn a_slide_formula_lives_in_a_part_of_its_own() {
        ...
        // pptx 不交这个键：LO 把 odp 的式子导成文本体里的 OMML（`a14:m` 套 `m:oMath`）
        // 加一张 EMF，那一族的量法还没做完，所以缺键而不是 0
        assert!(run("deck.pptx").get("equations").is_none());
    }
}

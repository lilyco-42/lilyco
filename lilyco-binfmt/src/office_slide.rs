...
pub struct OfficeSlide {
    ...
    /// 顺带把某一页上的一张表铺成 CSV（RFC4180：带逗号/引号/换行的字段加引号）
    #[arg(about = "Also render one slide table as CSV")]
    csv: bool,

    /// `--csv` 要哪一页：页名、部件名，或放映顺序里从 0 起的序号；不给就是第一页
    #[arg(about = "Slide for --csv: name, part or 0-based index", default = "")]
    page: String,

    /// `--csv` 要那一页的第几张表（从 0 起，按文档顺序；不给就是第一张）
...
/// `--page` 收三种写法：放映顺序里从 0 起的序号、部件名、页自己写的名字（odp 那一族）。
/// 挑不到就说这一共几页 —— 「没有这一页」与「这一页没有表」是两层事，不拿后一句顶前一句
fn pick_page(slides: &[Value], want: &str) -> Result<usize, String> {
    let want = want.trim();
    if want.is_empty() {
        if slides.is_empty() {
            return Err("这份放映一页也没有，没有可铺的表".to_string());
        }
        return Ok(0);
    }
    if let Ok(raw) = want.parse::<usize>() {
        return slides
            .get(raw)
            .map(|_| raw)
            .ok_or_else(|| format!("这份放映里没有第 {} 页（一共 {} 页）", raw, slides.len()));
    }
    slides
        .iter()
        .position(|one| {
            one.get("part").and_then(Value::as_str) == Some(want)
                || one.get("name").and_then(Value::as_str) == Some(want)
        })
        .ok_or_else(|| {
            format!(
                "--page 要的是从 0 起的序号、部件名或页名，收到「{}」（一共 {} 页）",
                want,
                slides.len()
            )
        })
...
/// 那一页的账要说是哪一页交出来的：号与身份一起钉回去（错误那条也钉，才知道挑到了哪一层）
fn stamp_page(ledger: &mut Value, page: &str, at: usize) {
    if let Some(one) = ledger.as_object_mut() {
        one.insert("page".to_string(), json!(page));
        one.insert("page_index".to_string(), json!(at));
    }
}

/// pptx 那一支的 `--csv`：表是页部件里的 `a:tbl`，被盖住的那一格照样在场、
/// 身上写 `hMerge` / `vMerge`（与 docx「整个不写」、ODF「另写一枚占位格」都不同）
fn pptx_csv(app: &OfficeSlide, bytes: &[u8], slides: &[Value], limit: usize) -> Value {
    if !app.csv {
        return Value::Null;
    }
    let at = match pick_page(slides, &app.page) {
        Ok(one) => one,
        Err(why) => return json!({"error": why}),
    };
    let page = match slides[at].get("part").and_then(Value::as_str) {
        Some(one) => one.to_string(),
        None => return json!({"error": "这一页没有部件名可指"}),
    };
    let Some(member) = xml(bytes, &page) else {
        return json!({"error": format!("这一页（{}）的部件读不出来", page)});
    };
    let root = xmlscan::parse_str(&member.as_text());
    let grids: Vec<crate::table_grid::Grid> = root
        .descendants("tbl")
        .into_iter()
        .take(limit)
        .map(|one| crate::table_grid::pptx(one, limit))
        .collect();
    let mut ledger =
        crate::table_grid::csv_of(true, &grids, &app.table, &format!("这一页（{}）里", page));
    stamp_page(&mut ledger, &page, at);
    ledger
}

/// odp 那一支的 `--csv`：表与 .ods 同一棵树（合并另写一枚 `covered-table-cell`），
/// 页的身份用 `draw:name` —— 没写名字的那一页不替它编一个。
/// `pages` 就是这一族建那本账走过的那一份 `draw:page` 清单，所以这里的序号与 `slides` 同源
fn odp_csv(app: &OfficeSlide, pages: &[&xmlscan::Node], slides: &[Value], limit: usize) -> Value {
    if !app.csv {
        return Value::Null;
    }
    let at = match pick_page(slides, &app.page) {
    ...
    let page = match slides[at].get("name").and_then(Value::as_str) {
        Some(one) if !one.is_empty() => one.to_string(),
        _ => format!("第 {} 页（这一页没写名字）", at),
    };
    let grids: Vec<crate::table_grid::Grid> = pages
        .get(at)
        .map(|one| -> Vec<crate::table_grid::Grid> {
            one.descendants("table")
                .into_iter()
                .take(limit)
                .map(|tbl| crate::table_grid::odf(tbl, limit))
                .collect()
        })
        .unwrap_or_default();
    let mut ledger =
...
fn run_office_slide(app: &OfficeSlide, ctx: &Context) -> Result<Value, AppError> {
    ...
    if doc.family == Family::Ooxml && doc.app == "powerpoint" {
        ...
        // 公式那一本按放映顺序逐页喂：页级那几格（形状数 / 式子数）与条目都在它自己那里算
        let mut eqrun = crate::equations::PptxRun::default();
        for entry in order.iter() {
            ...
            eqrun.add_slide(bytes, &slide_root, &part, &entry["show_index"], &all_rels);
        ...
        let result = json!({
            ...
            "order": order,
            "slides": slides,
            // 某一页上那张表铺成 CSV（挑页与挑表各说各的失败）
            "csv": csv,
            // 这一本按放映顺序逐页喂，最后一次性交出（页级那几格与条目都在它自己那里算）
    ...
    if doc.family == Family::Odf && doc.app == "powerpoint" {
        ...
        let csv = odp_csv(app, &pages, &slides, limit);
        let result = json!({
            "path": app.path.to_string_lossy(),
            "format": doc.format,
            ...
            "slides": slides,
            // 同一问在 odp 是这一族的 `table:table`（合并另写一枚占位格）
            "csv": csv,
            "size": size,
            "masters": masters,
            "layouts": layouts,
            ...
            // 这一页上有没有公式：一条式子一个 `Object N/content.xml`，而页缩略图也是 frame，两格分开数
            "equations": crate::equations::odp(bytes, &root, limit),
            "notes": notes,
        });
        ctx.done(result.clone(), start.elapsed().as_millis() as u64);
        return Ok(result);
...
mod tests {
    ...
    fn run_csv(name: &str, page: &str, table: &str) -> Value {
        let app = OfficeSlide {
            path: PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("tests/fixtures/office")
                .join(name),
        ...
        let (tx, _rx) = mpsc::channel();
        run_office_slide(&app, &Context::new_test(tx)).expect("office-slide --csv 应成功")
    }

    /// `--csv`：放映里那一页的那张表铺成 RFC4180。这一族的合并是第三种写法：
    /// 被盖住的那一格照样在文件里（pptx 在它身上写 `hMerge` / `vMerge`、ODF 另写一枚
    /// `covered-table-cell`），所以两家这里的 `columns_per_row` 都是齐的 —— 而「整个不写
    /// 那一格」是文档那一族 OOXML 的存法（同一张表在那边 `[2, 3]` 且 `ragged`，事实 115）
    #[test]
    fn a_slide_table_renders_as_csv_the_way_the_file_wrote_it() {
        let mine = run_csv("deck-tables.pptx", "ppt/slides/slide1.xml", "");
        assert_eq!(
            mine["csv"]["page"], "ppt/slides/slide1.xml",
            "挑中的那一页要自报身份"
        );
        assert_eq!(mine["csv"]["page_index"], 0);
        assert_eq!(mine["csv"]["table"], 0);
        assert_eq!(mine["csv"]["tables_total"], 1);
        assert_eq!(mine["csv"]["rows"], 3);
        assert_eq!(mine["csv"]["columns"], 3);
        assert_eq!(mine["csv"]["columns_per_row"], json!([3, 3, 3]));
        assert_eq!(mine["csv"]["ragged"], false);
        assert_eq!(mine["csv"]["empty_cells"], 2);
        assert_eq!(mine["csv"]["covered_cells"], 2);
        assert_eq!(mine["csv"]["cut"], false);
        assert_eq!(mine["csv"]["line_end"], "LF");
        assert_eq!(
            mine["csv"]["text"],
            "\"科目\n金额\",,备注\n服务器,124000,含税\n\"网络\n设备\",8000,\n"
        );
        // 同一张表在 LibreOffice 手里：那一页的整份账一字不差（合并换了写法也留着那一格）
        let rewritten = run_csv("deck-tables-lo.pptx", "", "");
        assert_eq!(
            rewritten["csv"], mine["csv"],
            "两家这一张要交同一份账：{rewritten}"
        );
        let odp = run_csv("deck-tables.odp", "表格那一页", "");
        assert_eq!(odp["csv"]["page"], "表格那一页", "挑中的那一页要自报身份");
        assert_eq!(odp["csv"]["page_index"], 0);
        assert_eq!(odp["csv"]["table"], 0);
        assert_eq!(odp["csv"]["tables_total"], 1);
        assert_eq!(odp["csv"]["rows"], 3);
        assert_eq!(odp["csv"]["columns"], 3);
        assert_eq!(odp["csv"]["columns_per_row"], json!([3, 3, 3]));
        assert_eq!(odp["csv"]["ragged"], false);
        assert_eq!(odp["csv"]["empty_cells"], 2);
        assert_eq!(odp["csv"]["covered_cells"], 2);
        assert_eq!(odp["csv"]["cut"], false);
        assert_eq!(odp["csv"]["line_end"], "LF");
        assert_eq!(
            odp["csv"]["text"],
            "\"科目\n金额\",,备注\n服务器,124000,含税\n\"网络\n设备\",8000,\n"
        );
        // 挑页三种接受法要落在同一页上：不给号 = 第一页，按号 = 按名
        assert_eq!(
            run_csv("deck-tables.odp", "0", "")["csv"],
            odp["csv"],
            "按号与按名"
        );
        assert_eq!(
            run_csv("deck-tables.pptx", "", "")["csv"],
            mine["csv"],
            "不给号"
        );
        let second = run_csv("deck.pptx", "1", "");
        assert_eq!(
            second["csv"]["page"], "ppt/slides/slide2.xml",
            "挑中的那一页要自报身份"
        );
        assert_eq!(second["csv"]["page_index"], 1);
        assert_eq!(second["csv"]["table"], 0);
        assert_eq!(second["csv"]["tables_total"], 1);
        assert_eq!(second["csv"]["rows"], 2);
        assert_eq!(second["csv"]["columns"], 2);
        assert_eq!(second["csv"]["columns_per_row"], json!([2, 2]));
        assert_eq!(second["csv"]["ragged"], false);
        assert_eq!(second["csv"]["empty_cells"], 0);
        assert_eq!(second["csv"]["covered_cells"], 0);
        assert_eq!(second["csv"]["cut"], false);
        assert_eq!(second["csv"]["line_end"], "LF");
        assert_eq!(second["csv"]["text"], "科目,金额\n服务器,124000\n");
        // 这份的第一页没有表：那句话要说清是「这一页」，不拿「这份文件」顶
        let empty = run_csv("deck.pptx", "0", "");
        assert_eq!(
            empty["csv"]["error"], "这一页（ppt/slides/slide1.xml）里没有第 0 张表（一共 0 张）",
            "页上没表要说哪一页：{empty}"
        );
        assert_eq!(
            empty["csv"]["page"], "ppt/slides/slide1.xml",
            "错误那条也带页身份"
        );
        // 页与表两层各说各的失败：号越界、串不是号
        let nine = run_csv("deck.pptx", "9", "");
        assert_eq!(nine["csv"]["error"], "这份放映里没有第 9 页（一共 2 页）");
        let bad = run_csv("deck.pptx", "没这页", "");
        assert_eq!(
            bad["csv"]["error"],
            "--page 要的是从 0 起的序号、部件名或页名，收到「没这页」（一共 2 页）"
        );
        let beyond = run_csv("deck-tables.pptx", "0", "3");
        assert_eq!(
            beyond["csv"]["error"],
            "这一页（ppt/slides/slide1.xml）里没有第 3 张表（一共 1 张）"
        );
        let notnum = run_csv("deck-tables.pptx", "0", "第三张");
        assert_eq!(
            notnum["csv"]["error"],
            "--table 要的是从 0 起的序号，收到「第三张」"
        );
        // 遗留 .ppt 不交这个键（那一族到不了页部件这一层）；不给 --csv 也不交
        assert!(
            run_csv("deck.ppt", "", "").get("csv").is_none(),
            ".ppt 做不出这张 CSV"
        );
    ...
    /// 这一页有哪些形状、哪个是组合：叠放序 = `spTree` 的孩子顺序，树自己不算形状，
    /// 组合自己那枚 `a:xfrm` 四份都在，而「字装在哪一层」在两族各有岔路。
    /// 期望值全部来自 `office_reader.py:slide_shape_tree_pptx` / `slide_shape_tree_odp`。
    #[test]
    fn a_group_is_one_row_whose_children_point_back_at_it() {
        let deck = run("deck-gr.pptx");
        let tree = &deck["slides"][0]["shape_tree"];
        assert_eq!(tree["available"], json!(true));
        assert_eq!(tree["family"], "ooxml");
        assert_eq!(tree["shapes_total"], 5);
        assert_eq!(tree["top_level"], 2, "一个散框 + 一个组合");
        assert_eq!(tree["nested"], 3);
        assert_eq!(tree["groups"], 1);
        assert_eq!(tree["max_depth"], 1);
        assert_eq!(tree["placeholders"], 0);
        assert_eq!(tree["unnamed"], 0);
        assert_eq!(tree["carriers_seen"], json!(["txBody"]));
        assert_eq!(tree["paragraphs_in_shapes"], 4);
        assert_eq!(tree["kinds_seen"], json!(["sp", "grpSp"]));
        assert_eq!(
            tree["distinct_names"],
            json!([
                "散着的框",
        ...
        let rows = tree["shapes"].as_array().expect("是数组");
        assert_eq!(rows.len(), 5);
        // `spTree` 自己那枚 `cNvPr id="1" name=""` 不是形状：第一条就是那个散框
        assert_eq!(rows[0]["name"], "散着的框");
        assert_eq!(rows[0]["id"], "2");
        assert_eq!(rows[0]["depth"], 0);
        assert_eq!(rows[0]["parent"], Value::Null);
        assert_eq!(rows[0]["text_carrier"], "txBody");
        // 组合：四份坐标都在，`off` 是 0,0 而 `ext` 与 `chExt` 一模一样（按写的交，不换算）
        assert_eq!(rows[1]["kind"], "grpSp");
        assert_eq!(rows[1]["id"], "3");
        assert_eq!(rows[1]["children"], 3);
        assert_eq!(rows[1]["text_carrier"], Value::Null);
        assert_eq!(rows[1]["paragraphs_direct"], 0);
        assert_eq!(
            rows[1]["xfrm"],
            json!({"off": {"x": "0", "y": "0"},
                   "ext": {"cx": "2900000", "cy": "900000"},
                   "chOff": {"x": "0", "y": "0"},
                   "chExt": {"cx": "2900000", "cy": "900000"}})
        );
        // 里面三条：`depth` 1，`parent` 指回组合在本清单里的序号
        for ix in 2..5usize {
            assert_eq!(rows[ix]["depth"], 1, "{ix}");
            assert_eq!(rows[ix]["parent"], 1, "{ix}");
            assert_eq!(rows[ix]["children"], 0, "{ix}");
        }
        // 第 2 页一个形状也没有：一串 0 与空表，而不是缺键
        let blank = &deck["slides"][1]["shape_tree"];
        assert_eq!(blank["available"], json!(true));
        assert_eq!(blank["shapes_total"], 0);
        assert_eq!(blank["shapes"], json!([]));
        assert_eq!(blank["kinds_seen"], json!([]));
        assert_eq!(blank["carriers_seen"], json!([]));
        assert_eq!(blank["max_depth"], 0);
        // LibreOffice 重写：形状、组合、两套坐标都保住，号整批重排、坐标走那条老换算、名字不动
        let back = run("deck-gr-lo.pptx");
        let btree = &back["slides"][0]["shape_tree"];
        assert_eq!(btree["shapes_total"], 5);
        assert_eq!(btree["groups"], 1);
        assert_eq!(btree["distinct_names"], tree["distinct_names"]);
        let brows = btree["shapes"].as_array().expect("是数组");
        assert_eq!(brows[0]["id"], "61");
        assert_eq!(brows[1]["id"], "62");
        assert_eq!(
            brows[0]["xfrm"]["off"],
            json!({"x": "100080", "y": "100080"})
        );
        assert_eq!(
            brows[1]["xfrm"]["chExt"],
            json!({"cx": "2899800", "cy": "899640"})
        );
        // 同一页转成 odp：层级与五个名字全对得上，但分组叫 `g`、号根本没有、尺寸自带单位
        let odp = run("deck-gr.odp");
        let otree = &odp["slides"][0]["shape_tree"];
        assert_eq!(otree["family"], "odf");
        assert_eq!(otree["shapes_total"], 5);
        assert_eq!(otree["top_level"], 2);
        assert_eq!(otree["nested"], 3);
        assert_eq!(otree["groups"], 1);
        assert_eq!(otree["kinds_seen"], json!(["custom-shape", "g"]));
        assert_eq!(otree["distinct_names"], tree["distinct_names"]);
        // 这一族的字直接挂在形状自己身上（没有一个 `draw:text-box`）
        assert_eq!(otree["carriers_seen"], json!(["self"]));
        assert_eq!(otree["paragraphs_in_shapes"], 4);
        let orows = otree["shapes"].as_array().expect("是数组");
        assert_eq!(orows[0]["id"], Value::Null);
        assert_eq!(orows[0]["text_carrier"], "self");
        assert_eq!(orows[0]["has_text_body"], json!(false));
        assert_eq!(orows[0]["paragraphs_direct"], 1);
        assert_eq!(
            orows[0]["size_written"],
            json!({"x": "0.278cm", "y": "0.278cm",
                   "width": "5.555cm", "height": "1.11cm"})
        );
        // 组合这一层一个尺寸属性都不写：空表，不是 0
        assert_eq!(orows[1]["kind"], "g");
        assert_eq!(orows[1]["size_written"], json!({}));
        assert_eq!(orows[1]["paragraphs_direct"], 0);
        // 一份 odp 里两种装法并存（`draw:frame` 用 text-box，custom-shape 用身上）
        let mixed = run("deck.odp");
        assert_eq!(
            mixed["slides"][0]["shape_tree"]["carriers_seen"],
            json!(["text-box", "self"])
        );
        assert_eq!(mixed["slides"][0]["shape_tree"]["paragraphs_in_shapes"], 4);
        // `nested` 两族不同义：这里 frame 套 image 也算一层，而 groups 是 0
        assert_eq!(mixed["slides"][0]["shape_tree"]["nested"], 1);
        assert_eq!(mixed["slides"][0]["shape_tree"]["groups"], 0);
        assert_eq!(mixed["slides"][0]["shape_tree"]["unnamed"], 1);
        // 一张 `pic` 不装字：carriers 是空表
        let pics = run("deck-pictures.pptx");
        assert_eq!(
            pics["slides"][0]["shape_tree"]["kinds_seen"],
            json!(["pic"])
        );
        assert_eq!(pics["slides"][0]["shape_tree"]["carriers_seen"], json!([]));
        // 遗留 .ppt 不交这个键（它的记录树没有形状树这一层）
        assert!(run("deck.ppt")["slides"][0].get("shape_tree").is_none());
    }

    /// 放映里的公式：一条式子一个部件，而页缩略图也是 `draw:frame` —— 「页上有几枚 frame」与
    /// 「几条式子」是两个数，不能互相顶替。
    /// 期望值全部来自 `scripts/acceptance/lyco_equations.py:odp_equations`。
    #[test]
    fn a_slide_formula_lives_in_a_part_of_its_own() {
        let hand = run("eqs.odp");
        let mine = &hand["equations"];
        assert_eq!(mine["family"], "odp");
        assert_eq!(mine["pages_total"], 2);
        assert_eq!(mine["frames_seen"], 3);
        assert_eq!(mine["frames_in_notes"], 0);
        assert_eq!(mine["page_thumbnails"], 0);
        assert_eq!(mine["objects_total"], 2);
        assert_eq!(mine["math_found"], 2);
        assert_eq!(mine["objects_without_math"], 0);
        assert_eq!(mine["parts_found"], 2);
        assert_eq!(mine["anchors_written"], 2);
        assert_eq!(mine["replacements_written"], 1);
        assert_eq!(mine["replacements_missing"], 0);
        assert_eq!(mine["block_written"], 2);
        assert_eq!(mine["inline_written"], 0);
        assert_eq!(mine["annotations_found"], 2);
        assert_eq!(mine["text_chars"], 4);
        assert_eq!(mine["equations_total"], 2);
        let rows = mine["items"].as_array().expect("是数组");
        assert_eq!(rows[0]["page"], 0);
        assert_eq!(rows[0]["frame_name"], "对象1");
        assert_eq!(rows[0]["anchor_written"], "as-char");
        assert_eq!(rows[0]["width_written"], "3.261cm");
        assert_eq!(rows[0]["object_part"], "Object 1/content.xml");
        assert_eq!(rows[0]["replacement_found"], json!(true));
        assert_eq!(rows[0]["text"], "ab");
        assert_eq!(rows[1]["text"], "12");
        assert_eq!(rows[1]["annotation_source"], "sqrt {1 2}");

        // 同一份让 LibreOffice 重写一遍：式子一条不少、字一字不变，而三格改了 ——
        // anchor 整个丢掉、每页多一枚装页缩略图的 frame、替位图引用了一张不在包里的部件
        let back = run("eqs-lo.odp");
        let rewrote = &back["equations"];
        assert_eq!(rewrote["frames_seen"], 3);
        assert_eq!(rewrote["frames_in_notes"], 2);
        assert_eq!(rewrote["page_thumbnails"], 2);
        assert_eq!(rewrote["anchors_written"], 0);
        assert_eq!(rewrote["replacements_written"], 2);
        assert_eq!(rewrote["replacements_missing"], 1);
        assert_eq!(rewrote["equations_total"], 2);
        let wrows = rewrote["items"].as_array().expect("是数组");
        assert_eq!(wrows[0]["style_written"], "gr1");
        assert_eq!(
            wrows[1]["replacement_target"],
            "./ObjectReplacements/Object 2"
        );
        assert_eq!(wrows[1]["replacement_found"], json!(false));
        assert_eq!(wrows[0]["text"], "ab");
        assert_eq!(wrows[1]["text"], "12");
        assert_eq!(
            mine["elements_seen"],
            json!([
                "math",
        ...
        // 一份没有公式的放映给 0（数过了没有），可它页上仍有 5 枚 frame、注块里 3 枚、
        // 页缩略图 2 枚 —— 把 frame 数当公式数就会在这里说谎
        let deck = &run("deck.odp")["equations"];
        assert_eq!(deck["equations_total"], 0);
        assert_eq!(deck["frames_seen"], 5);
        assert_eq!(deck["frames_in_notes"], 3);
        assert_eq!(deck["page_thumbnails"], 2);
        assert_eq!(deck["objects_total"], 0);
        // 图表走的是同一扇门：`Object N/` 这一族既装公式也装图表，判据是部件解析得出
        // `<math>` 根。`deck-chart.odp` 两枚 `draw:object` 的部件都在包里、都解析得开，
        // 而 `math_found` 仍是 0 —— 拿「有 draw:object」或「解析得开」当公式就会多数两条
        let chart = &run("deck-chart.odp")["equations"];
        assert_eq!(chart["frames_seen"], 2);
        assert_eq!(chart["objects_total"], 2);
        assert_eq!(chart["parts_found"], 2);
        assert_eq!(chart["math_found"], 0);
        assert_eq!(chart["objects_without_math"], 2);
        assert_eq!(chart["equations_total"], 0);
        assert_eq!(chart["elements_seen"], json!([]));
        assert_eq!(chart["replacements_missing"], 2);
    }

    #[test]
    fn a_slide_formula_is_written_twice_in_pptx() {
        let lo = run("eqs.pptx");
        let mine = &lo["equations"];
        assert_eq!(mine["family"], "pptx");
        assert_eq!(mine["equations_total"], 2);
        assert_eq!(mine["slides_with"], 2);
        assert_eq!(mine["fallbacks_written"], 2);
        assert_eq!(mine["duplicated_shapes"], 2);
        assert_eq!(mine["rasters_written"], 2);
        assert_eq!(mine["rasters_found"], 2);
        assert_eq!(mine["rasters_missing"], 0);
        assert_eq!(mine["paragraphs_total"], 3);
        assert_eq!(mine["text_chars"], 4);
        assert_eq!(mine["math_runs"], 4);
        assert_eq!(mine["alternates_total"], 4);
        let rows = mine["items"].as_array().expect("是数组");
        assert_eq!(rows[0]["holder"], "a14:m");
        assert_eq!(rows[0]["choice_requires"], "a14");
        assert_eq!(rows[0]["shape_id"], "9");
        assert_eq!(rows[0]["shape_name"], "对象1");
        assert_eq!(rows[0]["fallback_blip"], "rId1");
        assert_eq!(rows[0]["fallback_target"], "ppt/media/image1.emf");
        assert_eq!(rows[0]["fallback_found"], json!(true));
        // 同一个形状的第二遍写的是同一个号 —— 形状数与式子数因此差一倍
        assert_eq!(rows[0]["fallback_shape_id"], rows[0]["shape_id"]);
        assert_eq!(rows[0]["text"], "ab");
        assert_eq!(rows[1]["text"], "12");
        assert_eq!(
            rows[1]["structures"],
            json!(["rad", "radPr", "degHide", "deg", "e"])
        );
        let pages = mine["slides"].as_array().expect("是数组");
        assert_eq!(pages[0]["shapes_total"], 2);
        assert_eq!(pages[0]["formulas"], 1);
        assert_eq!(pages[0]["paragraphs_total"], 1);
        assert_eq!(pages[1]["shapes_total"], 3);
        assert_eq!(pages[1]["paragraphs_total"], 2);

        // 同一句问题的第二种写法：python-pptx 手挂的裸 `a14:m`，没有外壳也没有替身图
        let pp = run("eqs-pp.pptx");
        let plain = &pp["equations"];
        assert_eq!(plain["equations_total"], 2);
        assert_eq!(plain["alternates_total"], 0);
        assert_eq!(plain["fallbacks_written"], 0);
        assert_eq!(plain["duplicated_shapes"], 0);
        assert_eq!(plain["rasters_written"], 0);
        assert_eq!(plain["paragraphs_total"], 2);
        let prows = plain["items"].as_array().expect("是数组");
        assert_eq!(prows[0]["holder"], "a14:m");
        assert_eq!(prows[0]["in_alternate"], json!(false));
        assert_eq!(prows[0]["text"], "ab");
        assert_eq!(prows[1]["text"], "12");
        // 三格 fallback_* 是 null：这一族没写这一格，不是「写了没有」
        for key in ["choice_requires", "fallback_written", "fallback_found"] {
            assert!(prows[0][key].is_null(), "{key} 应当是 null：{plain}");
            assert!(prows[1][key].is_null(), "{key} 应当是 null：{plain}");
        }
        assert_eq!(plain["slides"][0]["shapes_total"], 1);
        // 两家的差别全在外壳：字、结构名、run 数一模一样
        assert_eq!(plain["structures_seen"], mine["structures_seen"]);
        assert_eq!(plain["text_chars"], mine["text_chars"]);
        assert_eq!(plain["math_runs"], mine["math_runs"]);

        // 没有公式的放映现在也交这一本（0 而不是缺键），页级三格照样各数各的
        let deck = &run("deck.pptx")["equations"];
        assert_eq!(deck["equations_total"], 0);
